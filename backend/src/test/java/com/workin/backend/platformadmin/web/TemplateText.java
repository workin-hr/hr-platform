 ...
 *
 * <p>Three of them parse the admin templates to hold a rule the compiler cannot
 * -- that every field is its own labelled cell, that a window the server opens
 * can be dismissed, that no page writes one id twice. Each had written its own
 * reader, and each was wrong in its own way about the same two things: that a
 * comment is not markup, and that a class attribute can be built rather than
 * written. #305's rounds 1 to 3 found four such misreads, every one of them
 * permissive. One reader serves them now, so the next one is fixed once.
 */
final class TemplateText {
	 ...
	 */
	static String rawClassOf(String tag) {
