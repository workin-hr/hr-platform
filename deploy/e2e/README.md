...
| `row-dialog`, `setting-templates`, `emp-picker`, `job-title-form`, `department-form`, `workforce-form`, `employee-form`, `employee-form-layout`, `employee-detail`, `company-detail`, `nav-drawer`, `org-filter-cascade`, `phone-form`, `attendance-form` | ✅ | ✅ | — | the dashboard's own scripts on a static page; they need no stack, so `npx playwright test --project=browser` runs them on their own |
