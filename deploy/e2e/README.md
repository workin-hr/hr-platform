...
| `row-dialog`, `setting-templates`, `emp-picker`, `job-title-form`, `department-form`, `workforce-form` | ✅ | ✅ | — | the dashboard's own scripts on a static page; they need no stack, so `npx playwright test --project=browser` runs them on their own |
