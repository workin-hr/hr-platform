...
| `web [--port 18100] [--host 127.0.0.1] [--out DIR] [--no-open]` | the same visit, in a browser, plus every step it can drive on its own button. It prints an address carrying a token it invents at startup, opens it, and serves nothing to anything else: the listener is loopback-only, every request but the page itself needs that token, a request arriving with another origin is refused, and a download resolves inside `field-report` or not at all. The page shows the transcript, the findings with their fixes, the results sheet as it fills and the files written, and its Stop ends a run exactly as Ctrl-C ends one in the terminal (same report, same restore checklist). The tools row -- read a terminal, back up its log, send `once`, check this laptop's network, scan the LAN -- runs a single step against one address while no visit is running, so a step that failed is repeated by itself instead of the whole visit being answered again. An address may carry its port (`127.0.0.1:14370`), which is how the lab simulators are reached |
