...
| `visit [--out DIR]` | the whole site visit, guided, in Egyptian Arabic, every question answered with a number (field-visit runbook, the quick path at the top). It asks before it scans, backs up a ZKTeco's log, runs the recorder in front of the lab receiver, allocates the terminal in the lab (`scripts/devices-lab.sh allocate`, with the time zone the operator reads off the terminal), times the push tests (and waits for a draining backlog first, so an old record cannot answer one -- counting uploads whose lines it could not read, which are still uploads; on a terminal whose line shape it cannot read at all it reports the latency as not measurable rather than timing whichever upload lands next), finds which record code carries in/out from a check-in punch and a check-out punch -- the code that changed -- sends through the agent twice, reads a Hikvision's event codes, imports a USB export, deletes the Hikvision password file, and writes `field-report/visit-SN-DATE.md`: the runbook's results sheet filled in and every problem with its fix, with no employee code, name or card number. Mode A (the lab) only: it has no path to production, which stays the runbook's section 12 and the repository owner's |
| `web [--port 18100] [--host 127.0.0.1] [--out DIR] [--no-open]` | the same visit, in a browser, plus every step it can drive on its own button. It prints an address carrying a token it invents at startup, opens it, and serves nothing to anything else: the listener is loopback-only and **a `--host` that is not loopback is refused before anything binds**, every request but the page itself needs that token, an answer names the question it answers and one naming a question the wizard has already left is refused, a request arriving with another origin is refused, and a download resolves inside `field-report` or not at all. The page shows the transcript, the findings with their fixes, the results sheet as it fills and the files written, and its Stop ends a run as a *single* Ctrl-C ends one in the terminal: the same report, and the restore checklist asked on the page and answerable there. Ctrl-C in the command that serves the page is the other case -- nobody is left at the page to answer, so it abandons the run, waits for the report and any tool still writing, and says if either did not finish. The tools row -- read a terminal, back up its log, send `once`, check this laptop's network, scan the LAN -- runs a single step against one address while no visit is running and while no other tool is, reusing the port, Comm Key and UDP the visit discovered for that address, refusing to send `once` until the in/out column is known, and asking for the customer's permission and which network before it scans one, so a step that failed is repeated by itself instead of the whole visit being answered again. An address may carry its port (`127.0.0.1:14370`), which is how the lab simulators are reached |
