...
في وضع A الأمر (`visit`) بيقرأ العمودين من بصمتين: دخول وخروج. لو الجهاز سجّل الاتنين بنفس الكودين
<span dir="ltr">(`punch=5, status=1` في الحالتين، زي `TERMINAL-A`)</span> فالبصمتين
...
في وضع B الأمر اللي بتشغّله هو `doctor`، وهو كمان بيقرأ السجل كله وبيطبع نفس الحكم
تحت آخر البصمات: سطر بيبدأ بـ <span dir="ltr">`in_out_field=`</span> وفيه توزيع
العمودين. **الحكم ده هو اللي بتمشي وراه، مش رقم آخر بصمة:**

1. الموظف يعمل بصمة **Check-Out** (زي تجربة 2 في 5.4).
2. شغّل `doctor` تاني ← بص على سطر <span dir="ltr">`in_out_field=`</span>.
3. لو السطر بيقول <span dir="ltr">`in_out_field=punch`</span> أو
   <span dir="ltr">`in_out_field=status`</span> ← ظبّط الملف على الاسم ده وشغّل `doctor`
   تاني. **اكتب النتيجة والتوزيع.**
4. لو السطر بيقول **«مش واضح»** ← **ماتشغّلش `once` خالص**، واكتب التوزيع في الـ issue.
   <span dir="ltr">`visit`</span> (وضع A) بيكمل ويبعت بـ <span dir="ltr">`in_out_field = "punch"`</span> لما العمود مايكونش واضح، وده مقصود: وضع A بيبعت لسيستم اللاب المحلي بس (<span dir="ltr">`server_url = https://localhost:18443`</span>)، فعمود غلط بيكلّف داتا اللاب لا أكتر، والزيارة بتسجّل الملاحظة على أي حال. أمر `once` في وضع B بيبعت للسيستم اللي الإعداد بيسميه، فده اللي ممنوع.
   **وماتغيّرش العمود عشان الرقم يطلع `1`:** على جهاز زي `TERMINAL-A` بصمة الخروج
   نفسها <span dir="ltr">`punch=5, status=1`</span>، فالعمود اللي رقمه `1` هو عمود
   **التحقق** مش الدخول/الخروج، ولو بعتّ بيه السجل كله بيتسجل بالعكس.
5. لو السجل أقل من 200 بصمة، `doctor` مايقولش حاجة (السجل مايكفيش) ← اعتمد على مقارنة
   الـ Push في النقطة اللي بعدها؛ ولو الجهاز مابيعملش Push، اكتبها في الـ issue
   وماتشغّلش `once`.
6. **لو الجهاز بيعمل Push كمان (عملت الخطوة 5):**
...
| <span dir="ltr">`[Errno 113] No route to host`</span> | مفيش ولا حزمة وصلت للجهاز، فالجهاز ماردّش عشان ماحدش سأله. التلات أسباب ليها نفس الـ errno: الـ IP غلط، الجهاز مقفول، أو الشبكة ماوصّلتش. **ماتفترضش إن الـ ARP هو السبب**: الأمر بيقول <span dir="ltr">`الـ ARP فشل`</span> بس لما يقيس <span dir="ltr">`arp = FAILED`</span> بنفسه | اقرا اللي الأمر كتبه في صف "حالة الشبكة وقت المشكلة"، وماتزوّدش عليه: لو مالقاش الـ interface اللي بيوصّل للجهاز، مافيش أرقام واي فاي وماينفعش تستنتج إن الواي فاي هو المشكلة. اتأكد الأول من الـ IP (صورة رقم 3) ومن إن الجهاز شغال. بعد كده، ولو الأرقام نفسها بتقول واي فاي ضعيف أو ping فيه مكررات، قرّب اللابتوب من الراوتر أو اتوصّل بكابل وكرّر الخطوة. **ماتكتبش ده في issue الأجهزة غير لما تتأكد إن الـ IP والجهاز تمام** |
