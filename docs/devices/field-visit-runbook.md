...
```bash
python3 -m workin_devices web
```

الأمر ده بيفتح صفحة على اللابتوب نفسه، وبيطبع لينكها. الصفحة بتوريك اللي بيحصل،
والمشاكل وحلولها، وورقة النتائج وهي بتتملي، والملفات اللي اتكتبت — كله في شاشة
واحدة. والأسئلة بتبقى **أزرار** تدوس عليها بدل ما تكتب رقم.

تحت في الصفحة فيه صف **أدوات**: كل خطوة لوحدها (اقرأ الجهاز، نسخة احتياطية،
إرسال once، افحص الشبكة، دوّر على الأجهزة). لو خطوة وقعت، تعيدها هي بس من غير ما
تعيد الزيارة كلها من الأول. الأدوات دي بتشتغل وقت ما الزيارة مش شغالة.

جنب زرار الأدوات فيه خانات بتتملي لوحدها من اللي الزيارة لقيته: **Comm Key**
و**UDP** و**عمود الدخول/الخروج** (punch ولا status) و**الشبكة**. سيبها زي ما هي
إلا لو إنت عايز تغيّرها:

- الجهاز اللي عليه Comm Key أو بيتكلم UDP مش هيرد من غير الخانتين دول.
- **إرسال once مش هيشتغل قبل ما عمود الدخول/الخروج يتحدد.** لو اتبعت بالعمود
  الغلط، الدخول بيتسجل خروج، ولو صلّحته بعدين السجل كله بيبان جديد وبيتبعت تاني.
- لو اللابتوب على أكتر من شبكة (كابل العميل + واي فاي + VPN)، اختار الشبكة اللي
  فيها الجهاز؛ الصفحة مش بتختار لوحدها. الشبكة الكبيرة بتتقلّب /24 حوالين
  اللابتوب، زي ما الزيارة بتعمل بالظبط. ونتيجة البحث بتتكتب في ملف
  `scan-*.json` جنب باقي ملفات الزيارة.
- **البحث في الشبكة مش بيشتغل غير بعد ما تعلّم على «العميل وافق على البحث»**،
  زي ما الزيارة بتسألك بالظبط (الخطوة 12.1). الزرار مش طريق يلف حوالين السؤال.

لما تقفل الأمر بـ Ctrl-C والزيارة لسه شغالة، **استنى**: الصفحة بتوقّف الزيارة
وبتسيبها تكتب التقرير وتطبع الخطوات اللي بترجّع إعدادات الجهاز قبل ما تقفل.

الصفحة دي بتشتغل على اللابتوب بتاعك بس. `--host` بأي عنوان تاني **مرفوض** قبل ما
حاجة تفتح: الصفحة بتقرا أجهزة العميل وبتبعت بتوكن الـ agent، وده مش بيتعرض على شبكة.

ولو مش عايز الصفحة، نفس الزيارة بالظبط بتشتغل في التيرمنال:
