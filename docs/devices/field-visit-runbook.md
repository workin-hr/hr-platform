...
7. إحنا **مش بناخد بصمات الصوابع ولا صور الوش**. لو الجهاز بعتها، السيستم بيرميها.
   ...
   بيه: سطور الحضور (`ATTLOG`)، والإعدادات (`OPTIONS`)، وسطور العمليات (`OPLOG`)،
   ونتايج الأوامر؛ ومن الماركات التانية JSON أو XML بس. أي حاجة تانية (بصمات، صور،
   بيانات تسجيل الموظفين، البطاقات، مهما كان اسمها) **مابتتكتبش**، وأي نص طويل
   مشفّر جوه اللي اتكتب بيتشال. في ملف الـ `.json` بتاع الطلب هتلاقي
   `request_body_withheld`: نوع اللي اتشال وحجمه بس. لو لقيت في `field-report/`
   صورة أو بصمة لأي سبب، امسحها وماتنقلهاش.
...
7. **(اختياري)** سجّل اللي الجهاز بيبعته لوحده:
   ...
   - الـ capture هيرد على الجهاز بـ `503`، فالجهاز هيحتفظ بالأحداث ومش هتضيع على
     العميل. إحنا بس بنسجّل شكل البيانات: أجزاء الـ JSON أو XML، من غير الصور
     (قاعدة 7).
...
- لو الجهاز فيه إعداد "server" أو "cloud": في Terminal 3 شغّل الـ `capture` من
  ...
  الـ capture هيكتب الـ JSON أو XML بس؛ لو الجهاز بيبعت شكل تاني، هتلاقي في ملف
  الـ `.json` الـ path والـ headers ونوع الـ body وحجمه، وده كفاية نعرف منه الجهاز
  بيتكلم إزاي.
