...
| `workforce_planning` | **Confirmed 2026-08-04**: `dashboard/pages/workforce_planning/` exists as a real page directory | Yes — headcount targets | No | Session; JWT; N/A | Desktop: HR-role/company-admin, company-scoped | Desktop: create/update/save_target/summary contract | Preserve desktop contract exactly. **Not tenant-safe despite the company scoping** — `save_target`/`update` accept unvalidated foreign branch/department/job-title ids and the name joins carry no tenant predicate, disclosing another company's names (D-131, `hr-legacy#33`). **This describes legacy, which is unchanged; the Java port stopped reproducing it on 2026-09-23 (D-277)**, so preserving the desktop contract exactly stops at the three org ids, which the port validates | Yes (desktop) |
