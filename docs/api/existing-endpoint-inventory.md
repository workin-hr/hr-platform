...
| `register_company.php` | POST | Step one: account holder, phone, password. Its duplicate probe is an **exact** phone match, so the same number in another spelling is not detected. Wraps its INSERT in an `ensureColumn()` helper that would run `ALTER TABLE` at request time from this public endpoint — unreachable against the frozen schema (every column exists) and **not ported** (D-135). |
| `complete_company_registration.php` | POST (multipart) | Step two. **Unauthenticated, takes `company_id` from `$_POST`, and returns a company-admin token for it** — see R-016 and the corrected threat-model row. Uploads both files *before* validating the three foreign keys, and those three share one `field_required` with no field name. `first_name`/`last_name` are written only when non-empty. |
| `get_company_registration_options.php` | GET | The three lookup lists for step two. The only GET in the module. |
| `register_employee.php` | POST | The older join path, keyed off the company's **phone** rather than its public code, creating an immediately-`accepted` employee. **It cannot succeed**: the INSERT omits `branch_id`, which is `NOT NULL` with no default, so it becomes `0` and `fk_employee_branch` rejects it. Every call is a 500 — see R-017. Superseded by `join_company.php`. |
| `join_company.php` | POST | The working join path: public code, first active branch, a `pending` inactive row, two notifications and an employee session token. Its duplicate probe is company-scoped and treats `rejected` as absent, so a rejected applicant passes it and is then stopped by the **global** `UNIQUE KEY phone` — a 409 from the INSERT's catch rather than a 400 from the probe. A caller whose phone is the company's own bypasses both global-uniqueness checks. |
| `lookup_company.php` | POST | Resolves a company by public code, falling back to the legacy id **only when the code is empty** — a supplied-but-invalid code is `company_code_invalid` and never falls back. |
| `check_status.php` | POST | Pre-login screen router: **all four outcomes are 200**, never a failure. Matches the phone exactly, so a differently-formatted number routes back to the company-code screen. |
| `login_company.php` | POST | Mobile company login. An unknown phone and a wrong password are both `invalid_phone_password`. An incomplete profile is **403**. A `pending` company **can** log in. Writes no onboarding notifications. |
| `login_desktop.php` | POST | Desktop login, branching on `login_as` (`hr`/`employee` vs `company`/`company_admin`, trimmed and lower-cased). Its HR branch is **HR only** and filters `role='hr' AND is_active=1` in SQL, ordering `e.id ASC` — oldest first, unlike every other login. Its company branch differs from `login_company.php` in three places: an unknown phone is `company_not_registered`, a wrong password is `incorrect_password`, and an incomplete profile is a **200 with no token**. It also writes the two onboarding notifications. |
