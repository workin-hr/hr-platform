...
- **Java port only (D-289):** `429 too_many_login_attempts` -- the phone
  (8 misses) or the client address (60 misses) has spent its 15-minute
  budget. Checked before the password, so even the right one is refused
  until the window passes. `login_company` and `login_desktop` share the
  same budget and answer the same way. Legacy has no limit.
...
| `check_status.php` | POST | Public, unauthenticated pre-login screen router by `phone`+`company_id` — **discloses an employee's role and active/company status without a password**. **All four outcomes are 200**, never a failure. Matches the phone exactly, so a differently-formatted number routes back to the company-code screen. |
...
| `request_phone_change.php` / `confirm_phone_change.php` | POST | Company sessions only, refusing anything else with **403** — where `delete_account_preview.php` refuses the same condition with 401. OTP-gated phone change with a global uniqueness check that uses `phone_sql_match_clause()` while the same-as-current check uses `phones_are_equivalent()`. Not subject to the DEBUG-disclosure finding (this pair never returned the OTP). **Confirming also sets `otp_verified = 1`**, so a company that never verified its original number becomes verified by changing it. Delivered in Wave 13.1a (D-134). |
