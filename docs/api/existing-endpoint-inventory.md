...
- **Java port only (D-289):** `429 too_many_login_attempts` -- the phone
  ...
  phone-wide 40. The phone budgets are keyed on the number's canonical
  E.164 form (D-291), so every spelling of one number -- `01012345678`,
  `+20 10 1234 5678`, `0020...`, Arabic-Indic digits -- spends one budget.
  `login_company` and `login_desktop` share the same budgets and answer
  the same way. Legacy has no limit.
- **Java port only (D-291, ADR-0020):** the phone is parsed by
  libphonenumber and matched as a number, not as text. Any human spelling
  finds the account -- spaces, hyphens, parentheses, dots, `+`, `00`,
  Arabic-Indic/Persian/fullwidth digits; a national number with no country
  is Egyptian, or the country stored on the account it matches (so a Saudi
  user typing `05...` still signs in). Input that is not a valid number --
  letters, circled or dingbat digits, an extension -- is answered as an
  unknown phone without a lookup. Legacy matched the column exactly.
...
| Endpoint | Method | Notes |
|---|---|---|
| `login_company.php` | POST | Mobile company-admin login by phone; gates on `otp_verified`, `profile_completed` and `status` in sequence. An unknown phone and a wrong password are both `invalid_phone_password`; an incomplete profile is **403**; a `pending` company **can** log in. Writes no onboarding notifications. Issues a plain JWT with **no `token_version` claim**, so it has no server-side revocation — see threat model. |
| `login_desktop.php` | POST | Single endpoint, two identities via `login_as` (`hr`/`employee` vs `company`/`company_admin`, trimmed and lower-cased). The HR branch is **HR only** — role must be exactly `hr`, with no `join_request_status` check unlike mobile employee login — and filters `role='hr' AND is_active=1` in SQL ordering `e.id ASC`, oldest first, unlike every other login. Only that branch issues a version-tracked token via `employee_issue_session_token()`. Its company branch differs from `login_company.php` in three places: an unknown phone is `company_not_registered`, a wrong password is `incorrect_password`, and an incomplete profile is a **200 with no token**. It also writes the two onboarding notifications. |
| `register_company.php` | POST | Step one of company onboarding: account holder, phone, password. Hashes the password, issues and sends an OTP; was subject to the DEBUG-disclosure finding (the Java port never returns the code, D-134). PHP's duplicate probe is an **exact** phone match, so the same number in another spelling is not detected -- which is how 16 pairs of companies share a number; the Java port refuses any spelling of a registered number and stores the national digits with the number's own dial code (D-291). Wraps its INSERT in an `ensureColumn()` helper that would run `ALTER TABLE` at request time from this public endpoint — unreachable against the frozen schema (every column exists) and **not ported** (D-135). |
| `complete_company_registration.php` | POST (multipart) | Step two: profile fields, logo and commercial-registration upload. **Unauthenticated — no auth or token check at all — takes `company_id` from `$_POST`, and returns a company-admin token for it** — see R-016 and the corrected threat-model row. Uploads both files *before* validating the three foreign keys, and those three share one `field_required` with no field name. `first_name`/`last_name` are written only when non-empty. |
| `get_company_registration_options.php` | GET | The three lookup lists (titles/activities/sizes) for step two's registration form. The only GET in the module. |
| `join_company.php` | POST | The working employee self-registration path, keyed on `companies.company_code`: public code, first active branch, a `pending` inactive row, two notifications, and an employee session token **auto-issued even while `join_request_status='pending'`**. Its duplicate probe is company-scoped and treats `rejected` as absent (the Java port compares canonical numbers and writes `country_code`, closing R-019 -- D-291), so a rejected applicant passes it and is then stopped by the **global** `UNIQUE KEY phone` — a 409 from the INSERT's catch rather than a 400 from the probe. A caller whose phone is the company's own bypasses both global-uniqueness checks. See duplication finding above. |
| `register_employee.php` | POST | The older employee self-registration path, keyed off the company's **phone** rather than its public code, creating an immediately-`accepted` employee. No token is issued; the caller must separately log in. **It cannot succeed**: the INSERT omits `branch_id`, which is `NOT NULL` with no default, so it becomes `0` and `fk_employee_branch` rejects it. Every call is a 500 — see R-017. Superseded by `join_company.php`. The Java port finds the company by any spelling of its phone (the one stored exactly as sent when two companies hold it), and validates the joiner's phone before the insert: `invalid_phone_number` for input that is not an Egyptian or internationally written mobile (D-291). See duplication finding above. |
| `lookup_company.php` | POST | Public, unauthenticated company directory lookup — **discloses company name, logo and status to any caller** who supplies either identifier. Resolves by public code, falling back to the raw `company_id` ("legacy_id") **only when the code is empty**; a supplied-but-invalid code is `company_code_invalid` and never falls back. |
| `check_status.php` | POST | Public, unauthenticated pre-login screen router by `phone`+`company_id` — **discloses an employee's role and active/company status without a password**. **All four outcomes are 200**, never a failure. PHP matches the phone exactly, so a differently-formatted number routes back to the company-code screen; the Java port finds any spelling of the number (D-291). |
| `forgot_password.php` | POST | Initiates a password-reset OTP. An unknown `type` is 400 and a known type with no account is **404**, so the endpoint tells an anonymous caller whether a phone is registered. Without a `company_id` the employee branch runs `resolve_single_employee_auth_by_phone()`, which **rejects a pending account** (`joined_company_wait_hr`) — so a pending employee can log in but cannot reset their password. Was subject to the DEBUG-disclosure finding; the Java port never returns the code (D-134). |
| `resend_otp.php` | POST | Issues an OTP to any caller-supplied number after a 60-second cooldown — it **never checks that the phone belongs to anybody**. Its cooldown answers the default **400**, not the 429 every other cooldown in the system uses, because `fail()` is called with no status argument. The Java port refuses a phone that is not a valid number with `invalid_phone_number`, and keys the code, the cooldown and the caps on the number's E.164 form, so a second spelling does not reset them (D-291). Was subject to the DEBUG-disclosure finding; the Java port never returns the code (D-134). |
| `verify_otp.php` | POST | Verifies an OTP for either registration completion or password-reset continuation (`purpose=password_reset` keeps the OTP alive for the follow-up call instead of clearing it). No attempt limiting. PHP's `otp_verified` update matches the phone **exactly**, unlike everything around it, so a company whose stored phone carries a `+` or a space verifies successfully and is never marked verified. The Java port keys every OTP on the number's E.164 form and marks the company holding the number -- the one stored exactly as typed when two companies hold it (D-291). |
| `reset_password.php` | POST | Consumes the still-active OTP from `verify_otp.php`; updates `password_hash` only — never bumps `token_version`, so existing sessions survive a password reset (see business-rule finding). **No minimum password length**, unlike `profile/change_password.php`'s six. The company branch updates **every** row whose phone matches a variant, not one (in the Java port: every company holding the canonical number, D-291). |
...
| `request_phone_change.php` / `confirm_phone_change.php` | POST | Company sessions only, refusing anything else with **403** — where `delete_account_preview.php` refuses the same condition with 401. OTP-gated phone change with a global uniqueness check that uses `phone_sql_match_clause()` while the same-as-current check uses `phones_are_equivalent()` (both a canonical-number comparison in the Java port, D-291). Not subject to the DEBUG-disclosure finding (this pair never returned the OTP). **Confirming also sets `otp_verified = 1`**, so a company that never verified its original number becomes verified by changing it. Delivered in Wave 13.1a (D-134). |
...
**Closed in the Java port 2026-09-23 (D-277).** The paragraphs above describe
...
authenticated `GET`. **The Java port scopes both the join and the subquery as of
2026-09-23 (D-277)**, so that sentence is legacy's behaviour and no longer the
port's.
