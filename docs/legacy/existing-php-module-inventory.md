...
| `workforce_planning` | 7 | Headcount targets: create/update/save_target/summary. All 7 endpoints individually documented in `docs/api/existing-endpoint-inventory.md`. **Not tenant-safe despite the `company_id` filter (D-131, `hr-legacy#33`)**: `save_target.php` and `update.php` accept unvalidated foreign `branch_id`/`department_id`/`job_title_id`, and the three name joins carry no tenant predicate, so another company's names can be read back. Still true of legacy; the Java port no longer follows it (D-277, 2026-09-23) |
