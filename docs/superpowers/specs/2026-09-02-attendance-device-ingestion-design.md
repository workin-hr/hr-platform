...
- The **ZKTeco connectivity pattern** is ADR-0006 **Part B**, which this
  ...
  (D-213), with one recorded condition: the hardware checklist in §4.3 must
...
- `backend/src/main/resources/db/phase1-mysql/phase1_extensions.sql`: the
...
**Decision (D-213, accepted 2026-09-02):** the ZKTeco adapter is the ADMS push receiver. The
...
`attendance_devices` (Phase-1-owned, `phase1_extensions.sql`; DDL
...
**Who may claim differs between pilot and production (D-214, R-072).** A
...
- **Recovery — required before broad rollout.** An **audited unclaim /
  ...
  to the wrong company today needs a manual database change, which D-214
...
**Gate semantics (D-214).** This checklist does not block merging Slice A
...
Two more, added by the review round (D-215) because the answers decide
...
**The Q5 audit is done (D-214) and the change is safe.** Every frozen-PHP
...
**`backend/src/main/resources/db/phase1-mysql/phase1_extensions.sql` is the
...
- **Two-hour minimum gap (Q2) — decided (D-214): a device punch is never
...
- **What the platform cannot fix in code: claim squatting** (R-072, Q8). The
...
- **No biometric templates on the platform in Phase 1 — decided (D-214).** `TransFlag`
...
| Slice | Scope | Gate |
|---|---|---|
| **A — ingest and visibility** | Registry and claim API; PIN identities; `/iclock/{cdata,getrequest,devicecmd}`; raw punch log with dedup; unclaimed sightings; heartbeat; template discard; property gate; raw-punch visibility endpoint; tests in §13 (the end-to-end test plays the device). **Not in A:** in-app per-serial rate limits (the pilot relies on the edge/reverse proxy), the command queue, a standalone simulator CLI | **Open** — D-213 accepted, Q1 = identity table, Q6 = tenant API (all 2026-09-02) |
...
| **B′ — ownership and recovery** (D-214) | Platform-mediated allocation of a device to a company, tenant assignment to a branch, and the audited unclaim / transfer / replace-device path | Required before production ingestion; closes R-072 |
| **C — device management** | Push `USERINFO` on employee create/move; time sync; queued-command UI with audit | Slice B live |
| **D — later** | Hikvision/Anviz/Suprema adapters; edge-gateway bridge for non-ADMS terminals. Biometric template sync is **not** on this roadmap — D-214 rules it out for Phase 1 | Separate decisions |

## 12. Decisions Required From The Repository Owner

Q0, Q1 and Q6 were decided on 2026-09-02 (D-213); Q2, Q3, Q5, Q7 and Q8 the
same day (**D-214**). None remains open — what is left is the work those
answers oblige, tracked in D-214's Follow-up, R-072 and §11's slice B′.

| # | Question | Recommendation |
|---|---|---|
| Q0 | Accept D-213: ADMS push is the primary ZKTeco adapter; the edge gateway is a fallback | **Decided 2026-09-02: accepted**, conditional on §4.3 passing on the first real device |
...
| Q8 | Proof of possession when claiming a device (**R-072**) | **Decided 2026-09-02: supervised tenant claiming for the pilot; platform-mediated allocation for production, with tenant HR only assigning an owned device to a branch; an audited unclaim/transfer/replace path before broad rollout** (§4.2, slice B′) |
...
- Added by the **independent review round** on PR #162, which found ten
  ...
  is refused.
- Added by the **second review round** (D-216), which found eleven more:
...
- **Decision-log numbering collision.** Two sessions are active in this
  repository today; D-213 is claimed here and must be renumbered if another
...
- `docs/bootstrap/decision-log-wave12r.md` D-213 (accepted 2026-09-02)
