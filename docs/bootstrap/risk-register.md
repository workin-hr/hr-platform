...
| Contingency | Revoke every platform-admin refresh family (`setStatusForPlatformAdmin` per admin, or the unwired `revokeAllForPlatformAdmin`), which forces every administrator to re-authenticate with their second factor. Cheap here in a way it would not be on the tenant surface: the population is small, internal and bootstrap-provisioned, so a mass revocation is an inconvenience rather than an outage. |
