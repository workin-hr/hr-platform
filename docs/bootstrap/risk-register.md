...
| Probability | High (currently true, not speculative) |
| Impact | High — every open pull request (#117, #118, #119, #120, #121) is unreviewable by Codex and unable to get a real (non-billing) CI signal until a human acts outside this repository. **Widened 2026-08-28 by D-121**: because Codex is now the named independent reviewer, an exhausted review quota makes affected pull requests **unmergeable**, not merely unreviewed — the workflow gate is unavailable rather than waived, and the contingency below (wait, then re-request) is the only path. |
| Severity | High |
