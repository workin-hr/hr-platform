...
| Contingency | **Platform admin**: deactivate the administrator rather than logging the session out — R-026's per-request check makes that immediate. Worth stating in an incident runbook, because it is the opposite of the intuitive action. **Tenant**: deactivating a platform admin does nothing for a stolen *tenant* token, and there is no equivalent per-request kill switch on that path. What is available is that tenant requests revalidate the selected membership on every request, so **revoking or suspending the membership** — not the logout — is what stops the token, and it stops it immediately. That asymmetry is the containment gap: the tenant surface, which carries the live write endpoints, has no single control an operator can reach for the way deactivation now serves platform admin. |
| Status | Open — recorded, not accepted. Surfaced 2026-08-31 by an independent security review of PR #152, then **widened by a second round of the same review** which pointed out that the tenant path shares the shape — a sentence I had written into ADR-0005 myself while assessing urgency on platform admin alone. Confirmed against the code: the `sid` claim is issued and never read, and `revokeAllForPlatformAdmin()` has no production caller. Deliberately **not** fixed inside #152, which exists to close R-026; changing logout semantics is a product-visible behaviour change that deserves its own decision rather than riding along in a security fix. |
...
## R-033: The BFF Credential Store Holds Every Live Platform-Admin Refresh Token

| Field | Value |
|---|---|
| Description | **ADR-0014** moves platform-admin token custody off the browser and into a server-side BFF session store, so that store holds the **raw refresh token** for every logged-in platform administrator, in a form the BFF can replay. That is the design working as intended — the tokens have to be usable to be refreshed — but it concentrates into one store what was previously distributed across browsers. A read of that store, or of any replica, snapshot or backup of it, yields every live platform-admin session at once. |
| Category | Security / Credential custody / Platform-admin surface (**ADR-0014**, Decision 2) |
| Probability | The exposure is certain by construction the moment the BFF ships; what is uncertain is whether the store is ever read by someone who should not. Not yet realised, because the BFF does not exist. |
| Impact | Platform admin is the **highest-privilege surface in the system**. A reader of this store needs no password and no second factor to **obtain a session**: they hold refresh tokens that mint access tokens directly, so the **login-time** MFA check is bypassed — that factor was spent at login and the token is what remains. It is *not* bypassed entirely, and overstating it here would contradict Decision 4: a destructive operation still requires a recent, single-use step-up bound to that exact operation, target and request digest, which a stolen refresh token does not carry. So the realistic reach is every administrator's **session** — reads, and any action not behind step-up — with the destructive surface still gated unless the compromise also yields an unconsumed matching approval. That is a meaningful limit and it is also exactly why Decision 4's bounds are not optional. Blast radius is every administrator logged in at that moment, not one. Backups make it worse than the live store, because a backup is read by more people, kept longer, and rarely watched. |
| Severity | **High** once the BFF ships. This is the cost ADR-0014 explicitly accepts — it trades a browser-storage risk for a server-side-custody responsibility — and a trade that is recorded but untracked is the half that gets forgotten. |
| Owner | Repository owner, with the BFF's deployment owner once one exists. |
| Mitigation | **None yet — nothing to mitigate until the BFF exists**, which is why this is recorded now rather than left to discovery. Required before it ships, and these belong in the store selection (prerequisite 2), not after it: encrypt refresh tokens at rest with a key held **outside** the store, so a database or backup read is not sufficient; restrict access to the BFF runtime identity alone; hold backups and replicas to the same standard as the primary; build a **genuine mass-revocation path** — there is none today. `revokeAllForPlatformAdmin(Long)` is named for *all sessions of one administrator*, takes a single id, and is unwired; it must not be mistaken for a global control during an incident (see Contingency). Whatever ships must invalidate every administrator's families in one operation, and be tested; and log reads of the store as a detection surface. The **absolute non-renewable family cap** the ADR already requires bounds how long a stolen token stays useful, which is a second reason that number is not optional. |
| Trigger | The BFF's session store being selected — the decision that determines whether these controls are cheap or retrofitted. Any replication, managed-backup or log-shipping configuration added to it later. |
| Contingency | **There is no global kill switch, and an earlier version of this row implied there was.** `revokeAllForPlatformAdmin(Long)` is named for *all sessions of one administrator* — it takes a single id and delegates to `setStatusForPlatformAdmin`, which updates that administrator's families only. It is also unwired, with no production caller. During a store compromise every administrator's token is exposed at once, so containment requires **enumerating every platform admin and revoking each**, which today is an ad-hoc script written under incident pressure against an API that was never exercised for this. Either a genuine global operation ships, or the enumerate-and-revoke procedure is written down and **tested** before it is relied on. Whichever is chosen, the revocation is only a delay and not a stop until per-request family-status enforcement lands — see the note in Mitigation. What remains cheap is the re-authentication itself: the population is small, internal and bootstrap-provisioned. |
| Status | **Open — recorded at design time.** Raised by independent review of PR #148, which observed that the ADR states this exposure in its Consequences while the canonical register tracked nothing for it. |
| Target Date | Before any BFF session store is chosen. It constrains the choice, so recording it afterwards would be recording it too late. |
| Evidence | `docs/adr/ADR-0014-platform-admin-web-authentication.md` Decision 2 (the BFF holds the token pair) and its Consequences (the BFF's session store enters platform-admin security scope). `PlatformAdminRefreshTokenRepository.setStatusForPlatformAdmin` is the per-admin revocation that exists; `revokeAllForPlatformAdmin` has no production caller, which is what makes the contingency a plan rather than a control today. Related: **R-024** (signing-secret custody) and **R-027**, which is **open** — the property this contingency would depend on does not hold yet, which is why the contingency above is written as a delay rather than a stop. |
| Last Reviewed | 2026-08-31 |
