...
| Owner | Repository owner. |
| Mitigation | **None yet.** The fix is to stop emitting `label` from the options serializer, or to record a decision permitting it. Not folded into the router work (**D-147**) that found it: that change is about paths nothing serves, and this is a delivered endpoint's response body. |
