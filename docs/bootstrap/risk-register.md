...
| Impact | Platform admin is the **highest-privilege surface in the system**. A reader of this store does not need a password or a second factor: they hold refresh tokens that mint access tokens directly, so **MFA is bypassed entirely** — the factor was spent at login and the token is what remains. Blast radius is every administrator logged in at that moment, not one. Backups make it worse than the live store, because a backup is read by more people, kept longer, and rarely watched. |
...
| Contingency | Revoke every platform-admin refresh family (`setStatusForPlatformAdmin` per admin, or the unwired `revokeAllForPlatformAdmin`). **This does not end access immediately, and the entry previously implied it did.** **R-027** is open on this branch: revoking a family stops *rotation*, but every access token already issued stays valid until its own `exp` — currently up to **900 seconds**. So a mass revocation during a store compromise leaves an attacker holding a working token for up to that window, on the highest-privilege surface in the system, and the containment is a delay rather than a stop. Closing R-027 (per-request family-status enforcement, **D-149**, PR #154) is what turns this contingency into an actual control; until that merges, plan on the residual window. What is cheap here regardless is the re-authentication itself: the population is small, internal and bootstrap-provisioned, so forcing everyone back through their second factor is an inconvenience rather than an outage. |
