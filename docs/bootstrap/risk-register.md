...
## R-037: Frozen PHP's `advances` Actions Mutate Another Company's Advance — Approve, Reject, Pay And Delete Are Unscoped

| Field | Value |
|---|---|
| Description | `apis/api/advances/approve.php`, `reject.php`, `pay.php` and `delete.php` write with `WHERE id = ?` and **no company predicate at all**, and none of them performs an ownership preflight. The authenticated caller's `company_id` is read and used for `requireCompanyActive()`, then never applied to the statement that mutates. Any authenticated company admin or HR user can therefore approve, reject, part-pay or delete **any advance on the platform** by id. `update.php` is the one endpoint of the five that does check ownership first, and it correctly answers 404. |
| Category | Security / Tenant isolation / Data integrity — **frozen-PHP defect, not present in the Java port** |
| Probability | Certain and trivially reachable. Advance ids are small sequential integers; the harness's foreign advance was `id=2`. No crafting, no race, no special role beyond the ordinary company-admin or HR login the product hands every customer. |
| Impact | Unauthorised **financial state changes across tenants**, not merely disclosure. Measured on the harness as a company-214 admin acting on advance `id=2`, owned by an employee of company 21: `approve` moved it `pending -> approved` (200); `reject` moved it `pending -> rejected` (200); `pay` moved `remaining` from `5050.00` to `5049.00` (200); `delete` **removed the row entirely** (200). Each was a fresh reseed, so the four effects are independent. An approved advance is a payroll deduction, so this reaches money, and `delete` destroys the record of it. |
| Severity | **High for PHP, none for Java.** The same four requests against Java returned **404** and left the row untouched in every case — `LegacyAdvanceService` resolves through company-scoped store methods. This is strictly more serious than **R-036**, which is a read-only disclosure; this one writes and deletes. |
| Owner | Repository owner. |
| Mitigation | **None applied, deliberately.** `hr-legacy` is frozen at `d113204` and **D-058** places the burden of proof on changes to the port, not on legacy. The exposure ends when PHP stops serving these routes, which is the cutover this port exists to enable. Recorded here because it is an open issue **in the system serving customers today**, and that fact should not be lost in a document about the port. |
| Trigger | Any cutover-date decision; any security review of the live platform; any customer report of an advance that changed state or vanished without an explanation. If PHP must stay live, the fix in each of the four files is the same missing predicate the sibling `update.php` already applies. |
| Contingency | A read-only query joining `advances` to `employees` and comparing each advance's owning company against the actor recorded for the change would identify whether this has been exercised — if the access log retains the actor. Deleted rows are not recoverable from the table itself and would need a backup. |
| Status | Open — recorded, not accepted. Found 2026-09-01 by the parity harness's mutation sweep, from an `advances/approve` status difference (PHP 500 vs Java 404) on an unknown id. The 500 came from the same unscoped re-read as **R-036**; investigating it exposed that the **write** is unscoped too. All five advances mutations were then tested individually against a foreign row rather than reasoned about, each from a fresh reseed. |
| Target Date | Before cutover, or sooner if the owner decides to patch frozen PHP. Given this one writes and deletes, it is the stronger candidate of the two for breaking the freeze. |
| Evidence | `hr-legacy/apis/api/advances/{approve,reject,pay,delete}.php` — `UPDATE`/`DELETE ... WHERE id = ?` with no `company_id`; `update.php` for the contrasting preflight. Harness measurements, 2026-09-01, company-214 admin against advance `id=2` (company 21), one reseed per case. Java: `LegacyAdvanceService` + `LegacyAdvanceStore` company-scoped lookups; all four answered 404 with the row unchanged. Related: **R-036** (the read-only sibling in `branches/update`); **D-058**. |
| Last Reviewed | 2026-09-01 |
