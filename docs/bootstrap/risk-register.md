...
| Probability | High (currently true, not speculative) |
| Impact | High — **whenever either quota is exhausted**, every open pull request becomes unreviewable by Codex and unable to get a real (non-billing) CI signal until a human acts outside this repository. **Widened 2026-08-28 by D-121**: because Codex is now the named independent reviewer, an exhausted review quota makes affected pull requests **unmergeable**, not merely unreviewed — the workflow gate is unavailable rather than waived, and the contingency below (wait, then re-request) is the only path. Measured during the 2026-08-24/27 outage, that meant five simultaneously blocked pull requests (#117, #118, #119, #120, #121); both quotas were restored on 2026-08-27 and nothing is blocked today, which is why this row is stated conditionally rather than in the present tense. |
| Severity | High |
