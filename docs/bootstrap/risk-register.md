...
| Field | Value |
|---|---|
| Description | `LegacyLeaveBalanceController.analyze` guarded its upload with `request.getPart("file")`, which returns **any** multipart part with that name — including a plain text field carrying no filename. PHP guards the same endpoint with `isset($_FILES['file'])` **and** `error === UPLOAD_ERR_OK`, and a part with no filename is a `$_POST` field there that never reaches `$_FILES`. Java therefore parsed the text as a spreadsheet and answered **200 "Leave balances file analyzed"** where PHP answers **400 `no_file_uploaded`**. A **second shape** diverged for a related reason: a part present with `filename=""` is `UPLOAD_ERR_NO_FILE` in PHP, but Spring still exposes a `MultipartFile` for it, so it reached the analyzer and answered `Empty or unreadable file` instead. |
...
| Owner | Repository owner. |
| Mitigation | **Applied 2026-08-31.** `analyze` now resolves the upload through `LegacyPostFields.file(request, "file")`, the shared helper the other four multipart endpoints already use — it skips parts whose `getSubmittedFileName()` is null, the servlet equivalent of PHP's `$_FILES` test, and brings PHP's field-name normalisation and last-duplicate-wins rule with it. The now-unused private `part()` helper was deleted so the divergent path cannot be reintroduced by a new call site. **The empty-filename half was missed in the first attempt** and added after independent review pointed at `LegacyAttendanceImportService`, which already carried the correct three-part test — a null part, a null `getOriginalFilename()`, or an empty one. Deliberately not an emptiness test on the bytes — a zero-byte file the user actually chose is `UPLOAD_ERR_OK` in PHP and must fall through to the format check, which is now pinned by its own test. |
...
| Status | **Closed — fixed 2026-08-31**, in two rounds: the text-field case first, then the empty-filename case after independent review showed the first mitigation overstated parity. |
| Target Date | Met. |
| Evidence | **Measured against both stacks, not inferred**, across four request shapes. Before: text field named `file` → PHP 400 `No file uploaded`, Java **200**. After, all four byte-identical: text field → 400 `No file uploaded`; `filename=""` → 400 `No file uploaded`; zero-byte file with a real name → 400 **`Empty or unreadable file`** (the boundary a careless fix would swallow); genuine CSV upload → 200 `Leave balances file analyzed`. Regression tests `LegacyLeaveBalanceEndToEndTest#aTextFieldNamedFileIsNotAnUpload`, `#anEmptyFilenameIsNotAnUploadEither`, `#aZeroByteFileWithARealNameFallsThroughToTheFormatCheck` and `#aRealUploadIsStillAccepted`; the first was **verified to fail against the pre-fix controller and pass against the fixed one**. The endpoint previously had no end-to-end coverage. Contract recorded in `docs/api/existing-endpoint-inventory.md`. |
