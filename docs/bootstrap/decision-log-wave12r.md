...
| Related | **#214** (item 7), D-198, D-197. |
...
| Corrected 2026-09-17 (#273) | **Scope** said that "the one supplier decides for all five toolbars". Four did: attendance, departments, employees and job titles ask `AdminViewModelAdvice.orgFilterCascade()`, which reads the advice's session. The requests toolbar was decided in `AdminRequestsController` from the administrator view that controller builds for itself, so a session bound to one company, once one arrives through the advice, would still have been sent every company's request types. The controller now decides from the advice's session too, so that one session decides for all five; `AdminRequestsControllerTest` pins it. The kept filter values were asserted only on the employees and requests toolbars; `AdminAttendanceEndToEndTest`, `AdminDepartmentsEndToEndTest` and `AdminJobTitlesEndToEndTest` now assert theirs. Both were found by #272's review. With `main`'s controller, exactly the controller test failed; each template keeping `0` for a filtered branch or department failed exactly its page's test. |
