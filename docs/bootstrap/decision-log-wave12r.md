...
| Divergences from legacy | Legacy's form also carries `data-fixed-company` on an edit and on a filtered add, so its script re-renders the select from active departments only, which drops a retired department the title has, and an unchanged save then clears it. The port leaves that attribute out: those forms keep the list the server rendered. This is the same safeguard D-240 keeps for attendance's retired exception types. |
