...
| Divergences from legacy | None: no rendered byte changes. Legacy issues its own five queries for the same payload (`org_helper.php:337-449`); the port issuing one is not a behaviour difference, and the maps, their group keys and their order are asserted unchanged. |
