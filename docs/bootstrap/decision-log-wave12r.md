...
| Decision | With a company chosen, the attendance add form and edit dialog offer only that company's **active** exception types, as legacy does (`payroll_list_helper.php:118-128`). With no company chosen, they still offer **none**. That is a deliberate divergence from legacy, which lists every company's active types. |
