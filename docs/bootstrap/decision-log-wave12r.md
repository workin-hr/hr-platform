...
| Evidence | ADR-0015's Implementation Status table; `PlatformAdminFullFlowTest` exercises enrol → login → MFA → session → step-up → action → logout over real HTTP against real Postgres. |
