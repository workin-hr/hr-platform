...
| The override taken so far | One. **#183, `930ea394`, 2026-09-09**, on owner instruction after the situation was put to them: `validate` and `test` green, `independent-review` failing because no round had landed on that head, GitHub reporting `BLOCKED`, merged with an admin override. Justification: the PR closes a PII disclosure serving every employee's last-login IP to any HR session, two money defects in payroll batch statistics, and a drift gate that had been reporting green while stale — all live on production paths. Recorded on the PR as `#183 (comment)` before the merge. |
