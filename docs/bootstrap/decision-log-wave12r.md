...
| Decision | Every hand-written helper in the dashboard that stands in for legacy's `(int)` cast of a request value now reads it with `PhpCast.intval`, PHP 8's cast, which `PhpCastTest` holds to PHP 8.3.33's own answers (D-249). The cast reads the leading decimal or exponent number, so `"1e2"` is 100, `"1.9e1"` is 19, `"5abc"` is 5 and `"abc"` is 0. **Where it applies:** the list filters' page, page size and ids (`DashboardListFilters`, `CompanyListFilters`), the company filter (`DashboardOrgScope`), a branch's radius and a department's branch ids (`Branch`, `Department`), payroll's run, row id, month and year, activities' page and page size, requests' type, leave balances' year, the employee detail page's id, month and year, attendance's aggregate page, the employees page's filter and form ids, an employee's contract duration, a company's activity, title and size, and the sort order, phone length and category of guide videos, phone countries, FAQs and banners. Each helper keeps its own rule for an absent or blank value. **Bounds.** A value bound for an `int` field or column is bounded to the `int` range with `Math.clamp`, where PHP's 64-bit value would reach legacy's insert or query. **Offsets.** A page number can now reach `Integer.MAX_VALUE`, and its offset does not fit an `int`, so `DashboardPage.offsetFor`, its `from` and `to`, and the activities page's offset are `long`. **Gate.** `AdminPhpIntCastTest` fails on any `Integer.parseInt` or `Long.parseLong` in `platformadmin`. Its one allowed use, `AdminDeviceView.id`, prints an id the database returned. |
