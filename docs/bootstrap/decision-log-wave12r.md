...
| Why one image | An image built differently for production is an image production is the first to run. Each profile pulls in `phase1-mysql` through a `spring.profiles.group`, so an operator passes one name and cannot get half the pair. |
...
| Rollback | Delete `deploy/`, remove `"deploy"` from `PHASE1_UNLOCKED_DIRS`, and drop the three profile files. Nothing in the application depends on any of it; the default profile is untouched and still means the PostgreSQL Phase-2 path. |
