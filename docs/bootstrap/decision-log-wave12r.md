...
| Retired targets | Unchanged. The service accepts only an active branch, department and job title, as legacy's `$validateWpPayload` does. On an edit of a row whose branch or job title has since been retired, the maps (active rows only) do not hold it, and the script shows it as `#id`, as legacy's does; the save is refused until the row is re-pointed. The script also shows an **active** job title as `#id` when it is outside the lists the script builds. With no department and a branch whose departments hold job titles, it lists only those and falls back to the company's job titles only when that list is empty. An edit keeps and saves such a job title, but an add under that branch cannot pick it until a department holding it is chosen. Legacy's script and maps do the same. |
