...
| Divergences from legacy | Three. **The maps' reach**, above. **Departments under a branch** are listed only when the department and the branch belong to the same company. `department_branches` carries no company and legacy does not check, and a department listed under another company's branch is one the service refuses on save. **Unused attributes:** legacy also emits `data-departments-by-company` and `data-placeholder-dept`, which `workforce-form.js` never reads; the port leaves them out. |
