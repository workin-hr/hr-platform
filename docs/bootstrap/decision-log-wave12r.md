...
| Corrected 2026-09-19 (#256) | **A write that changes nothing is a divergence, not an exact copy.** The "Per page" row reads as exact for companies, branches, departments, job titles and shifts, and it is not for a write that changes nothing. On those pages legacy's flash depends on the row count from `dbUpdate()` or `dbSoftDelete()`, and PDO counts *changed* rows. Re-approving a company that is already active, or re-rejecting one already rejected with the same reason, flashes nothing (`companies/page.php:11-13`). Saving a branch, job title or shift with its values unchanged flashes a red `error_required` and sends the operator back to the edit form (`branches/page.php:41-77`, `job_titles/page.php:41-67`, `shifts/page.php:32-58`). Deleting one that is already inactive, or a department that is already inactive, flashes the same red `error_required` on the list (`departments/page.php:62-71`). The port's connection counts *matched* rows, and `LegacyRowCountStartupCheck` refuses to start otherwise, because frozen endpoints read a zero count as a lost race. So each of these writes reports its row, is audited, returns to the list and flashes its success: `approved_ok`, `rejected_ok`, `saved_ok` or `deleted_ok`. Kept, because the row is in the state the operator asked for, and legacy's `error_required` tells them a required field is missing when none is. Three writes were never affected, because legacy ignores their count: a department's `save_edit` returns `true` after its update, a company's `save_edit` goes through `company_admin_update()`, which ignores the count, and a company's delete is a hard cascade. `anUnchangedSaveAndARepeatDeleteStillFlashTheirSuccess` in `AdminBranchesEndToEndTest`, `AdminDepartmentsEndToEndTest`, `AdminJobTitlesEndToEndTest` and `AdminShiftsEndToEndTest` saves each row twice with the same values, asserting both saves return to the list with their success, then deletes it twice. `PlatformAdminFullFlowTest.reapprovingOrRerejectingACompanyStillFlashesItsSuccessAndIsAudited` approves an active company and rejects a rejected one, and reads both flashes and both audit rows. |
