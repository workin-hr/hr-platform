...
| Divergences from legacy | Legacy swallows every failure of its tolerated deletes. A lock failure (`PessimisticLockingFailureException`, which Spring maps MariaDB's deadlock 1213 and lock wait 1205 to) is now rethrown, on this page and on the client `delete_account` path alike: InnoDB answers a deadlock by rolling the whole transaction back, so carrying on would commit the rest of the cascade without the rows deleted before it and without the audit row. Every other failure of those deletes is still tolerated. Counting differs too: a count that fails for a reason other than an absent table fails the page rather than reading zero. Rows reachable from the company only through a foreign key's `ON DELETE CASCADE`, and not deleted by a named statement, would not be counted; a check of every such key in `mysql_workin.schema.sql` found none. |
