...
| Retired targets | The service accepts only an active branch, department and job title of the row's company, as legacy's `$validateWpPayload` does; it requires neither the department to be linked to the branch nor the job title to belong to the department. `workforce-form.js`, like legacy's, builds each select from active rows only: the company's branches; the departments linked to the chosen branch, none until a branch is chosen; and the chosen department's job titles or, with no department, the job titles of the chosen branch's departments, with every job title of the company offered whenever that list is empty. A selected value missing from its list shows as `#id` and stays selected. **On an edit,** a retired branch, department or job title shows as `#id`, and the save is refused until the row is re-pointed; an active department not linked to the row's branch, or an active job title outside the list built for the row, shows as `#id` and saves unchanged. **On an add, or when an edit is re-pointed,** a department can be chosen only under a branch it is linked to, so one linked to no branch never can. A job title can be chosen only when an active department linked to the chosen branch lists it, or through the company fallback when the chosen branch's departments, or the chosen department, list none; before a branch is chosen the fallback offers every job title, but the save stays disabled, and choosing a branch rebuilds the list. So a job title with no department, or whose department is retired or linked to no branch, cannot be chosen at all when every branch of the company has linked departments and each of them lists at least one job title. Legacy's script and maps do the same. |
