...
| Decision | `rowDialog.jte` renders legacy's window, `<div class="modal-bg"><div class="modal">` with its `.modal-close` button, an `h2` title and a `form-footer` (the markup of `requests/page.php:143-163` and `advances/page.php:270`), instead of `<dialog class="row-dialog">`. `row-dialog.js` still fills the one window per page from the row's `data-dialog-*` values and announces `row-dialog:filled`, then opens it with legacy's `open` class. `crud.js` closes it on the close button and the backdrop, as it does every other modal, and `modal-a11y.js` gives it Escape, the Tab trap, focus on the first field and focus back to the opener. All fourteen row windows change at once -- advances (reject, edit), assets, attendance, companies and company detail (reject), complaints (reply), FAQs (category, item), leave balances, penalties, phone countries, and requests (approve, reject) -- because they all come from the one partial. |
| Why | #214 item 7. Legacy draws every window as `.modal-bg > .modal` -- 24 of them across its pages, including the decisions the port drew as dialogs (`requestDecisionModal`, `advanceDecisionModal`, `complaintModal`). The port drew the add form in legacy's modal and the edit or decision for the same row in a native dialog, so one page opened two different windows. D-198 chose the native dialog for its focus trap, Escape and backdrop; `modal-a11y.js` already supplies those for legacy's markup, so the reason no longer outweighs the difference. |
| Also fixed | `modal-a11y.js` meant to focus a window's first field rather than its close button, and did not. Its selector put `.modal` and a space in front of a comma-separated list and `:not(.modal-close)` after it, which scoped only the first and last alternatives, so the unscoped `button:not([disabled])` matched the close button that opens every window. Every modal on the dashboard opened with focus on its ×. The selector is now built alternative by alternative. |
| Divergences from legacy | **The subject line.** The window keeps the port's line naming the row the action is about (`.row-dialog__subject`), which legacy's windows do not have. **Keyboard.** Escape, the Tab trap and focus handling are `modal-a11y.js`'s, over legacy's markup; legacy has none of them. |
