...
| Bootstrap exception, stated plainly | The pull request carrying this correction cannot itself be gated by the mechanism it repairs -- until it is on `main`, no agent round can be counted on any head. `independent-review` is not a required context on `main` (only `validate` is), so this is a gap in evidence rather than a bypass of a required check, and it is recorded here rather than left implicit. Codex was quota-exhausted throughout (R-009), so the review was performed by a read-only reviewer agent under D-226 and its round is recorded on the pull request; this row is written to describe that, not to stand in for it. |
