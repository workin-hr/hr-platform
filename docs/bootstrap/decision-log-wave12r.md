...
## D-221: A Drift Check Is Verified By Its Output, Not Its Exit Code

| Field | Value |
|---|---|
| Decision | `scripts/check_all_legacy_drift.sh` runs all nine `hr-legacy` drift detectors with the flag each one actually takes, and **refuses to report success unless each proves it read `hr-legacy`**. A zero exit is no longer treated as evidence that a comparison happened. |
| The failure this closes | Three of the nine **degrade rather than fail** when they cannot find the source: they fall back to the port's own committed inventory, compare it against itself, print `not checked out`, and exit **0** with `OK` on the last line. Nothing about that output looks wrong at a glance. |
| How it was actually triggered | `moving-the-baseline.md` shipped with `--legacy` on the message check, which takes `--legacy-lang`. Python's `argparse` accepts an unambiguous **prefix** of a long option, so `--legacy` was silently read as `--legacy-lang` and pointed the script at `../hr-legacy` instead of `../hr-legacy/apis/lang`. It found no catalog, fell back, and exited 0. The review reported this as "exits with argparse status 2" — **the conclusion was right and the mechanism was wrong**, and the real behaviour is the more dangerous of the two: a checklist step that appears to pass while checking nothing. |
| Why a wrapper rather than a rule | Checking the other eight commands found a **second** wrong one the review had not flagged (`check_legacy_product_defaults_drift.py` takes no path argument at all). Two instances of one class, both found by review, neither by us. A class that survives a careful reader twice belongs in a command, not in discipline. |
| The rule, stated positively | Do not look for an error; look for the evidence that the intended source was read. **All nine must print a positive marker**, and `check_all_legacy_drift.sh` reports `UNPROVEN` and fails the run when one is absent — `route` and `message` must print `hr-legacy present:`, `product_defaults` must print `match hr-legacy at HEAD`, and the remaining six their own comparison markers, listed in `docs/migration/moving-the-baseline.md`. An earlier version of this row said a zero exit was sufficient for those six because they fail hard when the source is missing. That is true and still the first line of defence, but it is **not** what the wrapper enforces, and a maintainer following the weaker rule could have removed the marker check from six detectors and restored the false-green this decision exists to prevent. |
| Verified | Pointed at a directory with no `apis/` tree, the wrapper reports **6 failed, 3 degraded, 0 unproven** and exits 1. The split is exactly the documented invariant: the six that fail hard when the source is missing all fail, and the three that degrade to the port's own inventory all degrade — `check_legacy_product_defaults_drift.py` among them, since it now takes `--legacy` and so honours the selected checkout rather than its own sibling default. Against the real checkout it reports all nine reading `hr-legacy`, each having printed its documented positive marker. |
| Related | **R-071**, **D-220**, `docs/migration/moving-the-baseline.md` §4 and its "A gate's output is evidence, not a verdict" section, which this makes executable. |
...
| Decision | The independent review required by `AGENTS.md`'s Mandatory Workflow is performed by a **read-only review agent** invoked through the `degraded-independent-review` skill, which is renamed to `independent-review` since it is no longer a degraded path. `chatgpt-codex-connector[bot]` remains welcome and its rounds still count, but it is no longer the *only* party who can satisfy the gate. |
...
| Amendments this requires, per R-009's own rule | `AGENTS.md` Mandatory Workflow; **R-009**'s "not substitutable" clause; `docs/agents/responsibility-matrix.md`; `docs/bootstrap/manual-setup-checklist.md` step 5; and the skill's own name and framing. All in this branch. |
| **Bootstrap — D-226 does not authorize its own adoption** | **D-226 may not review or admit the pull request that introduces it.** #193 must be reviewed and admitted under the policy canonical **immediately before** D-226 — that is, with `chatgpt-codex-connector[bot]` as the named reviewer, or through **D-224**, which is that policy's own documented answer to the reviewer being unobtainable. Only **after** #193 merges may a review agent under D-226 satisfy the independent-review gate. |
| **No retroactive effect** | D-226 says nothing about pull requests merged before it became canonical, and confers no authority over them. A review performed before #193's merge is not a D-226 review and must not be recorded as one — including reviews of #190, #191 and #192, which are prerequisites of nothing here and must be admitted on their own terms. |
...
| Related | **D-121** (superseded in part), **D-222** (override, unaffected), **D-224** (the bootstrap route, and afterwards narrowed to "no qualifying reviewer obtainable at all"), **D-225** (whether the context becomes required again — now about a gate that can actually be satisfied), **R-008**, **R-009**. |
