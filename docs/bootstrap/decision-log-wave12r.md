...
| Regression coverage | `ListDisplayTest` holds both helpers to legacy's: the em dash for nothing, what fits shown whole, the cut with its ellipsis, Arabic counted by letters, an emoji never split, PHP's trim set rather than Java's, and, for the FAQ cut, the ellipsis counted inside the width with nothing trimmed. `AdminTableConventionsTest` reads the templates and fails closed: the actions header comes from the partial and no page writes its own; every clipped column cuts at legacy's length, and a call it cannot parse is a failure rather than a skip; every clipped cell carries the whole text in its `title`; no column clips by width as well; every status badge comes from `statusBadge.jte`, whose map is checked status by status against legacy's; and no pager sits inside its card. With round 1's `requests.jte`, exactly the badge and pager gates failed. Six mutants each failed exactly the test that pins them: pending painted grey, a cell losing its `title`, the FAQ cut taking 40, a clip written without its length, a pager put back inside its card, and the ellipsis counted outside the width. After the change, 683 tests in 62 classes under `com.workin.backend.platformadmin` passed. |
...
| Regression coverage | `ListDisplayTest` holds the helper to `hr_request_notes_display()`: the em dash for nothing, what fits shown whole, the cut with its ellipsis, Arabic counted by letters, an emoji counted as one character and never split, and PHP's trim set rather than Java's. `AdminTableConventionsTest` reads the templates: `everyActionsHeaderComesFromTheSharedPartial` fails for a template that writes its own header, `everyLongTextColumnCutsWhereLegacyCutsIt` compares the whole set of clipped columns and their lengths with legacy's own calls, so a new clip or a wrong length fails, and `noColumnIsLeftClippingWithCssWhereLegacyCutsTheText` fails for a converted column that also clips by width. With `main`'s `requests.jte`, exactly those three failed of 3. Four mutants each failed exactly the test that pins them: an advance's reason cut at 60 instead of 40, a converted cell keeping `tbl-clip`, the count taken in UTF-16 units, and Java's `trim()` in place of PHP's. After the change, 666 tests in 61 classes under `com.workin.backend.platformadmin` passed. |
| Rollback | Revert the merge. Nothing is stored. |
