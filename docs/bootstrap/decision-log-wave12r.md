...
| Divergences from legacy | **The radius input's `inputmode="numeric"`.** Not legacy's -- `_branch_form.php:30` carries no `inputmode` at all -- but the task asked for it, and it matches the port's own existing (also non-legacy) `inputmode="decimal"` on latitude and longitude, kept as a mobile-keyboard affordance rather than removed for being unsourced. |
