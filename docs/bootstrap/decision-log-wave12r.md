...
| Verified | Pointed at a directory with no `apis/` tree, the wrapper reports **6 failed, 2 degraded** and exits 1 — the two degraded being exactly the detectors that would otherwise have printed `OK`. Against the real checkout it reports all nine reading `hr-legacy`. |
