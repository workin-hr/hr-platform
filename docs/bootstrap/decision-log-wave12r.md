...
| Rollback | Revert the merge. Nothing is stored, migrated or sent: the retry re-opens a socket to the same address, the network facts run `ip neigh`, `ping` and read `/proc/net/wireless`, and the in/out change reads a log already downloaded. If `ip` or `ping` is missing the facts read `unavailable` and the visit carries on with the old generic remedy; if the retry is wrong in either direction the visible effect is a visit that waits four seconds longer, or one that fails as it does today. |
