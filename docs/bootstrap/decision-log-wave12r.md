...
| What the enforcement change does and does not buy | The gate now recognises an agent round through a **SHA-bound marker**, so a round cannot be recycled onto a head the reviewer never saw. That marker is **forgeable by anyone who can comment** — unlike a Codex round's `user.login`, which is not. It proves the round *names this head*; it does not prove a review happened. The compensating control is `check-review-dispositions.sh`, which now refuses when a round is claimed but no finding thread and no explicit `findings: none` declaration exists. Silence stops being a pass. |
...
| Related | **D-121** (superseded in part), **D-222** (override, unaffected), **D-224** (the bootstrap route, and afterwards narrowed to "no qualifying reviewer obtainable at all"), **D-225** (proposed, #192 — whether the context becomes required again; not on `main` at the time of writing), **R-008**, **R-009**. |
