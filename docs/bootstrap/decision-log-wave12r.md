...
| Values the form cannot show | Codex found these on the pull request. An unchanged save must not rewrite them either. **No code:** `employee_code` is nullable, and the list shows the id in its place (`CODE_SQL`). An employee stored without a code gets an empty, optional code input, and when it comes back empty the save leaves the column untouched. An employee who has a code still cannot have it blanked, and the add form still requires one. Legacy's form requires the code, so there such an employee cannot be saved without inventing one. **Zero dates:** legacy data holds MariaDB's `0000-00-00`; the development seed has 68. No date input can hold it, so a browser submits it as empty. An empty birth or hire date over such a stored value leaves the column untouched. The same shift with such a start date left empty adds no assignment. A date typed over one replaces it. Kept columns are left out of the `UPDATE` rather than written back, which also holds under a strict `sql_mode`. |
