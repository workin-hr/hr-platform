...
| Retired targets | The service accepts only an active branch, department and job title of the row's company, as legacy's `$validateWpPayload` does; neither requires the department to be linked to the branch. `workforce-form.js`, like legacy's, shows any selected branch, department or job title that is missing from the list it builds as `#id`, and keeps it selected. It builds its lists from active rows only: the company's branches; the departments linked to the chosen branch; and the chosen department's job titles, or, with no department, the job titles of the branch's departments, with the company's job titles used whenever that list is empty. So on an edit: **a retired branch, department or job title** shows as `#id`, and the save is refused until the row is re-pointed; **an active department not linked to the row's branch**, or **an active job title outside the list built for the row**, shows as `#id` and saves unchanged. An add cannot pick either until a branch or department that lists it is chosen. Legacy's script and maps do the same. |
