...
> on `feat/attendance-device-ingestion` while `main` independently allocated the
> same four numbers. `main`'s merged first, so the device decisions moved to
> D-213–D-216. Any older reference to D-158–D-161 in a device context means the
> entry now numbered 55 higher.

## D-213: ADR-0006 Part B resolved — ZKTeco terminals push over ADMS; the edge gateway is a fallback
...
## D-215: The independent review of Slice A — the resume stamp is never trusted, uploads are bounded by record count, and a punch remembers its branch
...
| Owner | Repository owner (merge); the fixes are this session's work under D-213/D-214. |
| Related ADR | ADR-0006 Part B (D-213); D-214 (the product decisions); **D-121** (the independent-review gate); **D-111** (why the deletion preview is not extended); **R-040**, whose mitigation list this corrects. |
...
## D-216: The second review round — device rows now follow the lifecycle of what they point at, and the receiver's own bounds are corrected
...
| Related ADR | D-213, D-214, D-215; **D-111** (why branch deletion cascades rather than refuses, and why neither deletion preview is extended); **D-078** (why the PIN binding is cleaned outside `CASCADE_TABLES`); **R-023**/Q7 (why every new lifecycle statement tolerates an absent table); **R-041** (attribution). |
...
| Impact | `LegacyEmployeeStore` and `LegacyBranchService` each gain one tolerant statement, neither changing its route's response; `device_operation_logs` gains a `dedup_key` and is written with `INSERT IGNORE`; `unclaimed_device_sightings` prunes on a genuinely new serial; `DeviceInput.isValidPin` is the single PIN rule; `devices.is_active_invalid` is a new message key in both catalogs. **Neither deletion preview is extended** — company or employee — for the D-111 reason already recorded in D-215. **R-041's attribution claim is corrected rather than defended**: legacy issues a `type=company` token for a company admin, which identifies the company and not a person, so `registered_by_employee_id` is null for those claims; an individual actor is recorded only for employee tokens, and real per-person attribution depends on F-26, which production claiming is gated on anyway. Device tests 65 → 72. |
| Follow-up | Unchanged from D-214/D-215: the §4.3 hardware checklist, Q7 provisioning, and slice B′ (platform-mediated allocation plus the audited unclaim/transfer/replace path) all gate production. |
...
## D-217: The Open-Session Deadline Follows The Baseline Commit, Not The One Before It
...
| The two-hour rule becomes a flag | Legacy refuses a check-in within 120 minutes of the last one and tells the app. A terminal cannot be told: it has already displayed "Thank you" and dropped the record from a finite buffer. Refusing would destroy the only evidence the person was present and show them nothing. So the punch is always stored and paired, and flagged `RAPID_RECHECKIN` for a human (D-214). The visible consequence -- the day split into two rows -- is what the reviewer is being asked to look at. |
...
| Related | **D-213** (the ownership boundary), **D-214** (the two-hour decision), **D-217** (the deadline this depends on being correct), **R-023** (provisioning both DDL files). |
