...
| Divergences from legacy | Three. **The maps' reach**, above. **Across companies:** a department is listed under a branch, and a job title under a department, only when both belong to the same company. `department_branches` carries no company, a job title's `department_id` is not held to its own company, and legacy checks neither; a department or job title listed across companies is one the service refuses on save. **Unused attributes:** legacy also emits `data-departments-by-company` and `data-placeholder-dept`, which `workforce-form.js` never reads; the port leaves them out. |
