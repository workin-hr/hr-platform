...
| Legacy parity, unchanged | The cascade itself is the same code in the same order. Uploaded files (logo, commercial registration) stay on disk, as in legacy. Device tables are deleted but not counted on the page, as D-164 and the mobile preview already do. There is no rollback of a committed delete: a database backup is the only way back, as in legacy. |
