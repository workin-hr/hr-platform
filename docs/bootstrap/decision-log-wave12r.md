...
| The deployment condition, stated because it bites | Uploads are written to `app.legacy-uploads.path` and referenced by `app.legacy-uploads.url`. Where those point at different hosts -- `compose.remote-db.yaml` with an absolute `UPLOADS_URL` at the PHP host is exactly that -- a logo uploaded here lands on this host and is served from the other, so it 404s. Creating companies from this surface needs both pointing at the same storage. |
