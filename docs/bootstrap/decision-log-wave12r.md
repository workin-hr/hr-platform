...
| Fails closed | An unreadable permission -- a 403, a rate limit, a renamed or deleted account, an unexpected response shape -- yields no permission, the round is **not** counted, and the reason is printed to the job log. A gate that cannot establish authority goes red. Covered by a regression case in both suites. |
...
| Bootstrap exception, stated plainly | The pull request carrying this correction cannot itself be gated by the mechanism it repairs -- until it is on `main`, no agent round can be counted on any head. `independent-review` is not a required context on `main` (only `validate` is), so this is a gap in evidence rather than a bypass of a required check, and it is recorded here rather than left implicit. It was independently reviewed like any other change. |
