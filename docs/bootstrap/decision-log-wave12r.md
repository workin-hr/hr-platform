...
| The rule, stated positively | Do not look for an error; look for the evidence that the intended source was read. `route` and `message` must print `hr-legacy present:`; `product_defaults` must print `match hr-legacy at HEAD`; the remaining six exit non-zero when the source is missing, so for those a zero exit is sufficient. |
