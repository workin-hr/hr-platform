...
| Also changed | **D-253's "Corrected 2026-09-19 (#256)" row** said, in the present tense, that the org services ignore the count and flash success "whatever the connection counts". That was true until this change and is now the opposite of what the code does, so the row is corrected and points here. **`LegacyRowCountStartupCheck`'s Javadoc** listed the frozen-PHP stores that depend on matched-row counting; it now lists these six writes too, and says what an operator would see if the guard were relaxed: an administrator told `no_data` about a row in front of them. **`AdminTemplateMessageKeyTest`** gains a gate for the keys a controller hands the page: the template scanner sees only `t.apply(errorKey)`, so a key invented in a `messageKey` switch was checked by nothing. It reads the arms, returns and pass-through `case` labels -- wrapped over lines or not -- of every `messageKey`, `errorKey` and `messageFor` method, every key a controller puts straight into the model as `errorKey`, including one chosen by an inline `switch`, and every key spelled into a redirect as `?error=`, and resolves each in both catalogues. #293's review found the first version missed three of those forms (round 2) and the next three (round 3), each time with a mutant that stayed green. **Two sources of keys remain outside it**, and the row says so rather than claiming the class is closed: the devices page builds `device_error_<suffix>` from a code the device agent sends, so no literal exists -- a second test holds that the family says the same in both languages, and #294 tracks pinning the codes -- and a `Form` or an `AdminService` outside the `web` package returns its own key for a controller to pass on, which #296 tracks reading. |
