...
| Divergences from legacy | **Attendance:** legacy's attendance page has no toolbar company filter. The port's had a number box, and #214 item 1 lists attendance among the pages to fix, so it gets the same select rather than losing the filter. **Ids:** the field now has legacy's id `org_company` on every page. The per-page ids (`br_company`, `jt_company`, and `emp_company_f`, which D-250 introduced) are gone; no test, spec or script referenced them, and the employee add form's own `#emp_company` is unaffected. |
