...
| Also changed | `Console` gained `title`, `note`, `enter` and `choose`; `Visit`'s four matching methods now delegate presentation and keep only their logic. `tests/test_visit.py`'s `ScriptedConsole` inherits `visit.Console` -- **one line, and the other 1 459 are untouched, which is the regression proof for the refactor**. `Visit.write_zk_agent_config` extracted from `agent_zk`. `Visit` keeps the in/out column it proved (`Visit.in_out`) instead of only writing it to the sheet, so a step repeated from a button sends on the column this visit established rather than a default (round 2). The address line is printed flushed: block-buffered through a pipe or a service log, the one line carrying the token would otherwise not appear until the process ended. |
