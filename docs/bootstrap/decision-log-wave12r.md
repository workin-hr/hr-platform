...
| Why this construction | The same one an SSH host key fingerprint uses, and safe for the same reason: HMAC is a pseudo-random function, so the digest reveals nothing about the key. Truncating to 64 bits keeps it far too short to serve as a signature while leaving collision between two real secrets a non-issue. The label is versioned so that a future scheme change cannot have an old fingerprint compared against a new one and read as a key mismatch. |
