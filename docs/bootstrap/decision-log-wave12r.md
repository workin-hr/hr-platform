...
| Divergences from legacy | **A shortfall keeps its minus sign.** Legacy prints the bare count and leaves the red of `--minus` to say it is a shortfall; the port prints "-30 دقيقة", so the column does not rest on colour alone. **The zero date is a dash.** PHP rolls `0000-00-00 00:00:00` over to "30 November -0001", and this refuses it; MariaDB holds only real dates or that one. The comment on `dayName` that said PHP parses a zero date to nothing was wrong, and is gone. **Kept on purpose:** legacy's `و` joins hours and minutes in English too, and its units are singular ("2 hour"); both are what legacy renders. |
...
| Regression coverage | `AttendanceDisplayTest` holds each helper to the output legacy's own functions printed under PHP 8.3: the month names in both languages and all twelve in Arabic, `HH:MM` with its zero, what `hr_parse_datetime()` accepts (`T` for the space, no seconds, a fraction, surrounding whitespace) and what it refuses (a bare date, nothing), the zero date as a dash, the weekday from Sunday, signed zero-padded hours, the duration in words with nothing or less as "0 دقيقة", and overtime in minutes with the shortfall's sign. `AdminAttendanceEndToEndTest.theRowsCarryLegacysDateTimeAndHoursFormats` renders the page over three employees, one over eight hours, one under and one exactly, and checks the wrapper, each cell's format, both badge classes with their text and the muted dash. `AdminLayoutWiringTest.everyPageWrapsItsContentInLegacysClasses` compares every page's `.content` classes with legacy's page by page, and fails on a page it has no entry for. With `main`'s `attendance.jte` and `payroll.jte`, exactly the end-to-end test and the wrapper gate failed. Nine mutants each failed exactly the test that pins them: payroll losing its wrapper, a shortfall painted with the plus class, March given the Levantine name a locale could supply, the time's hour unpadded, no fallback for a value without seconds, the weekday counted from Monday, worked hours unpadded, a negative duration not read as nothing, and the shortfall's sign dropped as legacy drops it. |
| Rollback | Revert the merge. Nothing is stored. |
