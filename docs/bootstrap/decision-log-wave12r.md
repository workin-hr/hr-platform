...
| Decision | With a company chosen, the attendance **add form** offers only that company's **active** exception types, as legacy does (`payroll_list_helper.php:118-128`). The **edit dialog** lists every one of the company's types, active first, with retired ones marked "Inactive", so editing a punch keeps a type retired after it was saved. Legacy's dialog loses that type (`attendance-form.js:196-198`): its select has no option for it, submits nothing, and the save clears it. With no company chosen, the **add form** offers **none**, a deliberate divergence from legacy, which lists every company's active types. The edit dialog then lists the types the page's rows carry, each named with its company and retired ones marked, so saving any listed row keeps its type. |
