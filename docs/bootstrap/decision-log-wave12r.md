...
| Corrected 2026-09-20 (#215) | **The scheme check is now `StoredUrl`, and it refuses one more thing.** The rule moved out of `EmployeeDetail.Document` so the company detail page and the companies list share it, and it now also refuses a protocol-relative value (`//host/path`, and the backslash form a browser reads the same way). Such a value names another origin while naming no scheme, so the branch that treats a schemeless value as a path on this host must not pass it through. A document URL stored that way is shown as an unlinked chip, as one with another scheme already was. |
