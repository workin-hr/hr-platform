...
> **Resolved later the same day by [D-146](#d-146-logout-revokes-the-access-token-not-only-the-refresh-family)**,
> which reads the `sid` claim on both surfaces and closes R-027.

## D-146: Logout revokes the access token, not only the refresh family
...
| Status | Accepted 2026-08-31. Closes **R-027**; completes the pair begun by **D-145**, which made *deactivation* immediate but explicitly left *logout* open. |
