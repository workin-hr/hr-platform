...
| Also fixed | **The table card was a containing block.** `app-ui.css` animates `.data-table-card` with `appFadeUp`, whose kept `transform` makes the card the containing block of every fixed element inside it. Legacy escapes that by writing its modals after the card; here the add modals and nine of the row windows sit inside it, so they were laid out against the card -- off-centre, scrolling the page on open, and on a short list with the title and × above the top of the document. The fix `admin-extra.css` already made for `.content` (the fade kept, the translate dropped) now covers the card too, under `app-ui.css`'s own selectors. The add modals on the same seven pages had the same fault before this change. **Three reject buttons were unstyled:** `btn-danger` is defined by no stylesheet. `modal-a11y.js` meant to focus a window's first field rather than its close button, and did not. Its selector put `.modal` and a space in front of a comma-separated list and `:not(.modal-close)` after it, which scoped only the first and last alternatives, so the unscoped `button:not([disabled])` matched the close button that opens every window. Every modal on the dashboard opened with focus on its ×. The selector is now built alternative by alternative. |
| Divergences from legacy | **The subject line.** The window keeps the port's line naming the row the action is about (`.row-dialog__subject`), which legacy's windows do not have; legacy names the company in the reject window's title instead, and shows the complaint's text in a preview block above the reply (`complaints/page.php:223`), which the port does not. **Keyboard.** Escape, the Tab trap and focus handling are `modal-a11y.js`'s, over legacy's markup; legacy has none of them. **Placement.** The windows stay inside their table cards, where legacy writes them after the card; the card no longer contains fixed elements, so they lay out as legacy's do. |
