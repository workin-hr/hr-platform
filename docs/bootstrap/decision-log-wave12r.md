...
| Divergences from legacy | **The asset text** was not clipped at all before, and its cell keeps the port's `bold` class; only the clipping and the `title` are added. **The FAQ list** keeps the port's CSS clipping (`tbl-clip`): legacy's FAQ table has no clipped column, so there is no length to copy, and the question column would otherwise stretch the table. |
| What does not change | What any column contains, the order of the columns, the row menus and their colours, the badges, and where the pager sits: all four already matched legacy. |
