...
| Not in this change | **The web page** (its own change, on top of this one): this is the field fix alone. **Windows' `ping`** counts with `-n` and prints another summary; `ping_facts` reports `unavailable` rather than parsing it, because a wrong `0%` would send an operator back to a terminal that is not the problem. **The errno is not threaded through `SourceError`/`DeviceResult`**: `unreachable_errno()` reads the `[Errno N]` token, which Python writes and does not translate, out of whatever the visit holds -- the exception, or the string an agent pass already flattened it into. Threading a field through the agent's result types would be the wider change, and the token is machine-written, not prose. **`READ_ONLY_COMMANDS` is untouched**; nothing here sends a terminal anything new. |
