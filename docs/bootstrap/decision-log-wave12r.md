...
## D-257: Every Terminal Path Lands In The Monolith, Through An Agent, A File Or A Push

| Field | Value |
|---|---|
| Status | Proposed 2026-09-16 in the pull request that adds `devices-agent/`; accepted when the repository owner merges it. The owner chose, in chat on 2026-09-16: keep hr-platform's device module and retire the standalone proof of concept (`poc-fingerprint-devices/workin-punch-gateway`, never in version control); support old and new terminals; a Python agent shipped as a Windows executable; and a production-database mode for the site visit. **The number is collision-prone** -- a parallel branch may claim D-257 first; renumber on merge if so. |
...
| Regression coverage | `DeviceAgentEndToEndTest` (real MariaDB): an agent's punches stored against its company's device with the employee resolved, re-delivery stored once, the same scan pushed and read as one row credited to the push, another company's, an unknown and an inactive serial answered identically with nothing stored, no token, a malformed one and a revoked one refused, an unreadable line quarantined, a batch above the cap refused whole, a USB delivery marked `FILE` and an unknown delivery refused, and a heartbeat that refreshes only its own devices and turns an unknown serial into a sighting. `AdminDevicesEndToEndTest`: allocation with its history and audit rows, a second allocation refused with a message and no audit row, a token rendered once and stored as a digest, revoked, and a USB export imported twice and stored once. `Phase1SchemaCheckTest`: a pre-agents database reported and repaired by the upgrade, which is harmless when re-run. `DevicesModuleIsolationTest`: every agent-surface bean gated by its flag. The agent: 35 unit tests, and pyzk reading the emulator exactly as the agent's client does over TCP and UDP, with and without a Comm Key. `scripts/devices-lab.sh simulate` runs all four paths against the local stack. |
