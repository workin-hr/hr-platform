...
| Rollback | Revert the merge. Nothing is stored or migrated, and the export only reads: the one new query is a single `SELECT` over `leave_balance` joined to `employees`, on the same `WHERE` the list already runs, bounded by one employee's row per year inside the filter (2,871 employees at the migration baseline), with no `LIMIT` because legacy has none. If it fails, the operator gets the dashboard's error page instead of a download and the request shows in the access log as a 500 on `/admin/leave_balances?export=csv`; nothing is written either way. |
