...
| .NET | Gateway | Evaluate during discovery | Depends on device discovery. Discovery note: `docs/devices/*.md` (vendor capability matrix, device/firmware inventory) were empty templates when this row was written; they are now populated, and the inventory's second entry is hardware-observed -- one ZKTeco MB20/ID read on a 2026-09-21 site visit (D-274). The blocker is therefore partly resolved: the pattern is verified on one real terminal, and the rest of the fleet's models are not read yet. **Update 2026-09-02**: D-164 (accepted) makes device-initiated ADMS push — no local component — the primary ZKTeco path and the .NET gateway a fallback for terminals without ADMS; the hardware checklist in `../superpowers/specs/2026-09-02-attendance-device-ingestion-design.md` §4.3 is the recorded condition before the adapter is declared verified. |
