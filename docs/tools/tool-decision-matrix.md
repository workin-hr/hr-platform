...
| Prometheus | Metrics | **Adopted, local only (ADR-0019)** | Registry in the app; scrape exposed in `local`/`integration` only. Production exposure still deferred — the endpoint is unauthenticated and the edge proxies every path |
