...
| [#14](https://github.com/workin-hr/hr-platform/issues/14) | PMR-08: All 10 architecture ADRs remain Proposed | High → Low | **Mostly done 2026-08-05** — 8 of 10 ADRs `Accepted` | Product + Engineering | **Done 2026-08-05** for ADR-0001–0008 (`docs/bootstrap/decision-log.md` D-016 through D-024; ADR-0002 and ADR-0006 each with an explicit per-part caveat). Remaining: ADR-0009 (one factual item — desktop-access-universality confirmation, not resolvable from this repository) and ADR-0010 (deliberately, fully open on all 6 dimensions) | Each ADR's `Status` moved to `Accepted` (or a considered `Rejected`/`Deferred`) by an explicit human decision, recorded in the ADR file itself | ADR-0009: as soon as desktop-access is confirmed. ADR-0010: no target, genuinely open |
...
| F-10 | Three real frontends exist (PHP dashboard, Flutter desktop, Flutter mobile), not two — dashboard-vs-desktop fate | High (architecture-level) | P1 (downgraded from P0 — no longer blocks any-implementation, only admin-surface module cutover) | Product | **Decision recorded** 2026-08-04 (Option E, role-based split); `hr-legacy#26` investigated and resolved non-blocking; Engineering sign-off (Next.js) and feature-disposition (`salary_calculator`, etc.) both resolved 2026-08-05; **only remaining item is confirming every current dashboard company/HR user has realistic desktop-app access** — a real-world fact this repository cannot confirm on its own | Tracked via `docs/adr/ADR-0009-dashboard-vs-desktop-admin-client.md`; closes when `Status` moves to `Accepted` (pending product/business-owner confirmation of desktop-access-universality, per that ADR's Validation Evidence) | Before ADR-0009 `Accepted`, before admin-surface module cutover (does not block the technical spike or non-admin backend implementation) |
