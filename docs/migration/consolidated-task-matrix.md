...
| [#4](https://github.com/workin-hr/hr-legacy/issues/4) | OTP code returned in API response when `DEBUG=true` | Critical (unconfirmed-live) | P0 — **the one exception to this section's non-blocking policy** | Security (verify) then Engineering | **Potentially blocking today, independent of migration timing**: if `DEBUG=true` is confirmed live in production, this is an active, currently-exploitable account-takeover path (`PMR-05`) that may warrant emergency action on the *legacy* system directly (e.g. flipping the config flag), separate from and faster than any migration timeline. The *new-system* acceptance criterion (never echo OTP under any config flag) remains non-blocking for backend-implementation start | Manual operator confirms live `DEBUG` value (screenshot/config export) — this specific verification remains urgent regardless of migration sequencing; Java rewrite never echoes OTP under any config flag | Live-value verification: immediately, independent of migration. New-system fix: before that module's cutover |
...
| [#13](https://github.com/workin-hr/hr-platform/issues/13) | PMR-07: Technical spike not yet executed | High | P0 (narrowed 2026-08-04) | Engineering | Blocking: any-implementation gate — **but the spike itself shrank from 10 days/6 hypotheses to 3 days/1 hypothesis** (tenant-isolation mechanism only; see `technical-spike-plan.md`'s Revision Summary) | Spike executed per the revised `technical-spike-plan.md`, H2 exit criteria evaluated and recorded | Before any backend implementation begins — now a 3-day, not 10-day, prerequisite |
