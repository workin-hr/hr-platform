...
| [#5](https://github.com/workin-hr/hr-legacy/issues/5) | Cross-tenant IDOR in API `advances` module | High | P1 (acceptance criterion) | Engineering + Security | **Non-blocking for implementation start.** Mandatory acceptance criterion for the advances module | Same tenant-isolation test suite covers `advances` approve/reject/pay/delete/create | Before that module's cutover |
...
| F-17 | ADR-0010 Task 4: permission-matrix test suite covering every role and capability | Medium | P1 | Engineering | Blocking for authorization-module completion | Real automated test suite; closes when every role/capability combination in `docs/architecture/authorization-model.md` §7 has test coverage | Before authorization module is considered implementation-complete |
| F-18 | ADR-0010 Task 5: negative cross-tenant tests for every sensitive module, modeled on `hr-legacy#2`/`#3`/`#5`/`#6` | Medium | P1 | Engineering | Blocking per-module | Real automated tests; closes per module as it's implemented | Before each sensitive module is considered implementation-complete |
| F-19 | ADR-0010 Task 6: tests proving revoked permissions stop working on the next request | Medium | P1 | Engineering | Blocking for authorization-module completion | Real automated test proving Dimension 5's guarantee; closes when it exists and passes in CI | Before authorization module is considered implementation-complete |
| F-20 | ADR-0010 Task 7: tests for inactive memberships and mismatched identity/membership combinations | Medium | P1 | Engineering | Blocking for authorization-module completion | Real automated tests covering Dimension 2's fail-closed matrix; closes when they exist and pass in CI | Before authorization module is considered implementation-complete |
...
| F-23 | ADR-0010 Task 10: architecture test detecting application use cases without explicit authorization-policy declarations | Medium | P1 | Engineering | Blocking for authorization-module completion | Real ArchUnit (or equivalent) test; closes when it exists, runs in CI, and is proven to fail on an undeclared use case | Before authorization module is considered implementation-complete |
