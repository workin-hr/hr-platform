...
| F-26 | Independent per-platform-admin account identity (individual login, MFA, distinguishable per-admin audit trail) — replacing the shared platform-admin password (`hr-legacy#11`) | Low (explicitly deferred by product decision, not a hidden risk) | P3 | Product + Engineering | **Non-blocking** — MVP keeps the shared-password model by direct product decision, 2026-08-05 (`docs/architecture/authorization-model.md` §7) | `hr-legacy#11`; closes when product schedules and Engineering implements individual platform-admin accounts | Backlog — no target date set |
