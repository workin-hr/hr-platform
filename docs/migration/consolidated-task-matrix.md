...
| [#4](https://github.com/workin-hr/hr-legacy/issues/4) | OTP code returned in API response when `DEBUG=true` | Critical — **CONFIRMED LIVE 2026-08-04** | P0 — **active incident, the one exception to this section's non-blocking policy** | Security (remediate now) then Engineering | **Blocking today, confirmed, independent of migration timing**: `DEBUG=true` is confirmed live in production by the repository owner directly — this is an active, currently-exploitable account-takeover path (`PMR-05`), requiring emergency action on the *legacy* system directly (flipping the live config value), outside this environment's reach (no production access). The *new-system* acceptance criterion (never echo OTP under any config flag) remains non-blocking for backend-implementation start | Live production `DEBUG` value changed to `false`, confirmed by whoever has that access; Java rewrite never echoes OTP under any config flag | Live-value remediation: immediately, overdue. New-system fix: before that module's cutover |
