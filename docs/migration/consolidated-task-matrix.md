...
| [#8](https://github.com/workin-hr/hr-legacy/issues/8) | `hr_permissions` matrix enforced on ~21 endpoints, ignored on ~130+ others | Medium | P1 | Engineering | Blocking: cutover of any permission-gated module | New system enforces permissions structurally per `docs/adr/ADR-0010-authorization-model.md` (Dimension 3/4 — the specific model and enforcement boundary are still open there, but structural-not-per-endpoint enforcement is already a fixed constraint) — architecture review confirms this before first module cutover | Before any module's cutover; the model itself before ADR-0010 is `Accepted` |
...
| [#13](https://github.com/workin-hr/hr-platform/issues/13) | PMR-07: Technical spike — **executed 2026-08-05, real result obtained** | High → Low (evidence now exists) | P0 → done (narrowed 2026-08-04, executed 2026-08-05) | Engineering | **No longer blocking** — 6/6 tests passing on a clean rebuild, recommendation recorded (RLS, non-superuser-role condition). Only ADR-0002 Part B's formal acceptance remains | `spike/tenant-isolation-spike/SPIKE-NOTES.md` (full findings); `docs/migration/technical-spike-plan.md` Exit Criteria — met | Before any backend implementation begins — the spike itself is done; only human ADR acceptance remains |
| [#14](https://github.com/workin-hr/hr-platform/issues/14) | PMR-08: All 9 architecture ADRs remain Proposed | High | P0 for the ADRs that gate implementation start; **most no longer need to wait on this issue as a block** | Product + Engineering | **Reframed 2026-08-04**: per-ADR classification now exists (`pre-migration-readiness-gap-analysis.md`) — ADR-0001, ADR-0005, ADR-0007, ADR-0008 recommended for immediate acceptance (no dependency); ADR-0003, ADR-0004, ADR-0009 recommended for decision now using existing evidence; only ADR-0002's tenant-isolation-pattern detail and ADR-0006's final vendor specifics remain spike/access-blocked. This issue tracks getting human sign-off on all 9, not a single monolithic blocker | Each ADR's `Status` moved to `Accepted` (or a considered `Rejected`/`Deferred`) by an explicit human decision, recorded in the ADR file itself | The 4 recommended-immediate ADRs: as soon as a human reviews and agrees. The rest: per their individual classification |
...
| F-10 | Three real frontends exist (PHP dashboard, Flutter desktop, Flutter mobile), not two — dashboard-vs-desktop fate | High (architecture-level) | P1 (downgraded from P0 — no longer blocks any-implementation, only admin-surface module cutover) | Product | **Decision recorded** 2026-08-04 (Option E, role-based split); `hr-legacy#26` investigated and resolved non-blocking; remaining work is Engineering sign-off + feature-disposition confirmation, not the core decision | Tracked via `docs/adr/ADR-0009-dashboard-vs-desktop-admin-client.md`; closes when `Status` moves to `Accepted` (pending Engineering sign-off on Next.js/JTE + dashboard-only feature disposition, per that ADR's Validation Evidence) | Before ADR-0009 `Accepted`, before admin-surface module cutover (does not block the technical spike or non-admin backend implementation) |
