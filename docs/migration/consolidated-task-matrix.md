...
| [#7](https://github.com/workin-hr/hr-legacy/issues/7) | JWT valid 10 years; no admin token revocation; password change/reset never invalidates sessions | High | P1 (acceptance criterion — design decided 2026-08-04, reclassified from P0) | Engineering + Security | **Non-blocking for implementation start**, now that the direction is decided: `docs/adr/ADR-0005-authentication-direction.md` records short-lived JWT + refresh + revocation + forced re-authentication as the confirmed approach, recommended for immediate acceptance. Mandatory acceptance criterion for the auth module specifically | ADR-0005 `Accepted` by a human decider; Java rewrite implements short-lived JWT + refresh + revocation per `docs/security/authentication-remediation-design.md` | Before auth module cutover |
...
| F-26 | Independent per-platform-admin account identity (individual credentials, sessions, revocation, audit attribution) — replacing the shared platform-admin password (`hr-legacy#11`) | High (real, confirmed security/audit gap; not accepted as MVP architecture) | **P0** | Product + Engineering | **Partially closed 2026-08-06** — individual identity, credentials, login, and structurally-separated JWT sessions shipped (`backend/src/main/java/com/workin/backend/platformadmin/`; `docs/security/threat-model.md`'s `platform_admins` table, own JWT issuer/audience, own `SecurityFilterChain`, no shared password anywhere in the new backend). **Still blocking**: individual session revocation and comprehensive audit attribution are not yet implemented — same explicitly-tracked gap already accepted for tenant-identity refresh/revocation (`JwtService`'s own Javadoc), not silently dropped here either. No `platform.*`-gated business functionality (approve/suspend/delete company) may reach production until revocation and audit attribution close | `hr-legacy#11`; identity/login/session-separation done — `backend/src/test/java/com/workin/backend/platformadmin/` (`PlatformAdminBootstrapTest`, `PlatformAdminAuthFlowTest`) and `backend/src/test/java/com/workin/backend/security/PlatformAdminDomainSeparationTest`, all passing in CI; remaining closure needs real session revocation + an audit-log system (tracked as follow-up, not yet a numbered item) | Revocation + audit attribution: before any platform-admin functionality reaches production |
