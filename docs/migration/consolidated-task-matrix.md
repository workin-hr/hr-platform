...
| F-13 | RLS-arm test-coverage gap from the H2 spike: no test yet proves RLS's fail-closed behavior when the tenant session-variable-setting call itself is omitted (spec fully written 2026-08-05, not yet implemented — implementation is out of `hr-platform`'s planning-only scope) | Medium (real acceptance criterion for the first module adopting RLS, not hypothetical) | P1 | Engineering | **Blocking**: the first backend module that adopts RLS must not be considered done without this test, per `docs/adr/ADR-0002-modular-monolith-baseline.md` Decision, condition 3 | Full Given/When/Then spec: `docs/adr/ADR-0002-modular-monolith-baseline.md` (Decision, condition 3) and `docs/migration/technical-spike-plan.md` ("Test Coverage Gap" subsection); closes when the real, automated test exists and runs in CI for the first RLS-adopting module | Before the first RLS-adopting module is considered implementation-complete |
...
| F-21 | ADR-0010 Task 8 = F-13 (same requirement, cross-referenced, not duplicated work) | — | — | — | See F-13 | See F-13 | See F-13 |
| F-22 | ADR-0010 Task 9: startup check that fails when the runtime PostgreSQL connection is a superuser or bypasses RLS | Medium (real, dangerous failure mode confirmed by the H2 spike) | P1 | Engineering | Blocking for any module using RLS | Real startup-time check; closes when it exists and a test proves it fails loudly on a superuser connection | Before the first RLS-adopting module is considered implementation-complete |
