...
| 8 | Legacy login ported branch-for-branch from `login_employee.php:50-119`; `TenantScopeFilter` binds/releases the authenticated tenant per HTTP request | `7030757` |
| 9 | `TenantAwareJpaTransactionManager` binds the filter on every transaction via a `NO_TENANT` sentinel (fail-closed: unscoped reads zero rows, not every tenant's); `TenantBindingEndToEndTest` proves it over real HTTP → MariaDB | `d5da4e6` |

## Next, in order

| # | Item | Why it's next |
|---|---|---|
| 10 | **Complete auth-level tenant isolation tests** | ADR-0012 item 3 calls for every `RlsFailClosedTest`/`TenantContextIsolationTest` scenario ported assertion-for-assertion. The RLS-fail-closed side is ported (`TenantFilterFailClosedTest`); `TenantContextIsolationTest`'s guarantee — a token's `membership_id`/`tenant_id` claims are never trusted without server-side re-validation — still lives only in the PostgreSQL-era `com.workin.backend.tenancy` suite and hasn't been re-proven against the new legacy login/tenant-scope path. |
...
PR #101's own description ("Still to land on this branch: ... Login/token
implementation is deliberately not started") predates items 7–9 and is
stale relative to its own later commits. Worth correcting in the PR body
before merge so a reviewer isn't misled about what's already landed.
