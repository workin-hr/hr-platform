...
| # | Item | Why it's next |
|---|---|---|
| 10 | **Complete auth-level tenant isolation tests** | Unblocked by #9 — there is now a real HTTP path to send a forged/tampered tenant claim through. ADR-0012 item 3 calls for every `RlsFailClosedTest`/`TenantContextIsolationTest` scenario ported assertion-for-assertion. The RLS-fail-closed side is ported (`TenantFilterFailClosedTest`); `TenantContextIsolationTest`'s guarantee — a token's `membership_id`/`tenant_id` claims are never trusted without server-side re-validation — still lives only in the PostgreSQL-era `com.workin.backend.tenancy` suite and hasn't been re-proven against the legacy login/tenant-scope path, which now exists to attack. |
