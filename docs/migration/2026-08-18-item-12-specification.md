...
two relationship findings from direct read-only production aggregates: zero
non-null department-manager references, and 1,897 job-title-department
references with zero missing or cross-company departments. No additional
security divergence is required.
...
| 12.3c | `phase1/item12-job-titles` | **Implemented and corrected after owner review — see D-062/D-065/D-067.** `LegacyJobTitle` + repository + service + controller + response view; all 5 endpoints; P-1a; active-department create/update validation; department cannot be cleared; inactive departments remain readable through the legacy left-join shape; correlated branch summary reproduced through the real department junction; shared `LegacyValues` coercion replaces service-local numeric approximations | Independently runnable coverage: `LegacyValuesTest` (17 tests), `LegacyJobTitleEndToEndTest` (17 real-MariaDB HTTP tests), and `LegacyJobTitleTenancyTest` (4 adapter/tenancy tests). Tests prove PHP scalar and array numeric casts, integer-bound saturation, active-row visibility, branch/department filters, inactive and branchless department reads, response wire shape, persisted DECIMAL rounding, repeatable soft delete, D-057's absent permission gate, and cross-tenant denial. Final requested gate `./gradlew clean test phase2Test --console=plain`: **BUILD SUCCESSFUL; MariaDB/default suite 71 classes and 504 tests; PostgreSQL migration suite 1 class and 12 tests; 0 failures, errors, or skips in both**. This final boundary is stacked on 12.3b; human re-review and merge remain pending |
