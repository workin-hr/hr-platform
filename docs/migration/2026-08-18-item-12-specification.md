...
graph TD
  ...
  EMP --> D
  MASTER --> ATT[Wave 12.6: attendance 15,<br/>schedules 3]
...
| **12.6** | `attendance` (15 endpoints) + `schedules` (3 endpoints) = **18** | **Scope corrected 2026-08-22 from source.** The module is `apis/api/schedules/`, not `employee_schedules`, and **`employee_shift_assignments` has no endpoint directory at all** -- it is written inside `employees/create.php` and `employees/update.php`, both already ported in Wave 12.4, and read by `attendance/list.php` and five helpers. Same shape D-054 found for `department_branches`: a table inside another module's transaction, not a module. First derived-tenancy consumers -- none of the three tables carries a `company_id`. |
...
| 12.6 | `phase1/item12-attendance-schedules` | `attendance` (15), `schedules` (3) | Calculation-parity tests vs legacy; employee-derived tenancy on all three tables; **D-091's bounded `WEEKLY_OFF_DAYS` reader is a precondition for 11 of the 18**, and `overall_report`/`export` are unschedulable until the broad payroll boundary is decided |
