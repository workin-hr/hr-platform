...
| 12.3c | `phase1/item12-job-titles` | **Implemented and corrected after owner review — see D-062/D-065/D-067/D-069.** `LegacyJobTitle` + repository + service + controller + response view; all 5 endpoints; P-1a; active-department create/update validation; department cannot be cleared; inactive departments remain readable through the legacy left-join shape; correlated branch summary reproduced through the real department junction; shared `LegacyValues` coercion replaces service-local approximations | Independently runnable coverage: `LegacyValuesTest` (19 tests), `LegacyJobTitleEndToEndTest` (21 real-MariaDB HTTP tests), and `LegacyJobTitleTenancyTest` (4 adapter/tenancy tests). Tests prove PHP scalar, array, and explicit string casts; raw `branch_id`/`department_id` query coercion before Spring conversion; integer-bound saturation; active-row visibility; inactive and branchless department reads; response wire shape; persisted `"Array"` names and DECIMAL rounding; repeatable soft delete; D-057's absent permission gate; and cross-tenant denial. Final sequential gate `./gradlew clean test phase2Test --console=plain`: **BUILD SUCCESSFUL; MariaDB/default suite 71 classes and 511 tests; PostgreSQL migration suite 1 class and 12 tests; 0 failures, errors, or skips in both**. This final boundary is stacked on 12.3b; human re-review and merge remain pending |
