...
| 12.3a | `phase1/item12-branches` | **Implemented — see D-060.** `branches`: `LegacyBranch` + repository + service + controller (`com.workin.legacy.organization`), all 6 endpoints (`list`, `one`, `create`, `update`, `delete`, `generate_qr`), P-1a; the location-coordinate heuristic ported literally as `LegacyBranchLocationResolver` (D-058: PHP is the specification for this logic, not an idealised redesign) | Per-module adapter + E2E parity vs legacy (36 new tests: 27 E2E, 9 tenancy/adapter), verified against real production data first (D-059) — `radius_meters = 0`, both-null lat/lng, `is_active = 0` all round-trip; **D-056**: delete reproduces legacy's explicit `branch_assigned_employees_count()` pre-check (409 before attempting the delete, proven not to short-circuit-after-attempting), atomic `department_branches` cleanup + hard delete, FK-violation catch retained only as a documented race-condition fallback; **D-057**: explicit test proving a principal with no `hr_permissions` row at all succeeds on every write, and that the role gate (unlike Wave 12.1) applies to reads too; **D-060 flags one real cross-tenant read in legacy's own `update.php`** (no ownership check before its response re-fetch, which has no `company_id` predicate at all) — not reproduced, per ADR-0011's existing security-posture exception; full suite 446 tests, 0 failures, both profiles |
| 12.3b | `phase1/item12-departments` | `departments` **and `department_branches` together — `department_branches` is not an independent module (D-054)**, it is part of `departments`' persistence contract: mutated only inside `departments`' own create/update transaction, never its own endpoint. `LegacyDepartment` + repository + service + controller, all 5 endpoints, P-1a on `departments`, P-1c (already built, Wave 12.2) proven against real `department_branches` rows for the first time | Per-module adapter + E2E parity; the `INNER JOIN` to `department_branches`/`branches` in every read (a branch-less department must not be readable — reproduces a real enforced legacy invariant, not an implementation detail); the transactional delete-then-reinsert of the branch set on create/update; **D-055**: explicit test proving `manager_id: null` does *not* clear an existing manager (the `COALESCE(?, manager_id)` bug, reproduced deliberately); delete is a **soft** `is_active = 0` flip with no dependent-row guard — do not reuse the existing PostgreSQL `DepartmentService`'s hard-delete shape; **D-057**: no `hr_permissions` gate |
