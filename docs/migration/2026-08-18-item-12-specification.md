...
| 12.3c | `phase1/item12-job-titles` | **Implemented and packaged — see D-062/D-064.** `LegacyJobTitle` + repository + service + controller + response view; all 5 endpoints; P-1a; active-department create/update validation; department cannot be cleared; inactive departments remain readable through the legacy left-join shape; correlated branch summary reproduced through the real department junction | Independently runnable coverage: `LegacyJobTitleEndToEndTest` (15 real-MariaDB HTTP tests) and `LegacyJobTitleTenancyTest` (4 adapter/tenancy tests). Tests prove active-row visibility, branch/department filters, inactive and branchless department reads, response wire shape, malformed numeric validation, persisted DECIMAL rounding on update, repeatable soft delete, no permission gate, and cross-tenant denial. Final requested gate `./gradlew clean test phase2Test --console=plain`: **BUILD SUCCESSFUL; MariaDB/default suite 71 classes and 492 tests; PostgreSQL migration suite 1 class and 12 tests; 0 failures, errors, or skips in both**. This final boundary is stacked on 12.3b; human merge remains pending |
