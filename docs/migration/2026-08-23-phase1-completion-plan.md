...
|---|---|---|
| `FINAL_COMPATIBLE` | **129** | Every delivered route, on its literal `/apis/api/**` URL. Exactly the set `LegacyPhpRouteInventoryTest` asserts bidirectionally (`hasSize(129)`). Waves 12.4 through 12.10, the Wave 12.R retrofit, Wave 12.6.6's two attendance endpoints, Wave 12.9's `payslips/export.php`, and **Item 13.0's `configs/get.php`** — the first endpoint delivered outside Item 12. |
...
| `ITEM13_REMAINING` | **69** | §2.2's 71 less `auth/login_employee` (Wave 12.R) and `configs/get.php` (Item 13.0). |
| **Live total** | **198** | 129 + 0 + 0 + 69 |
...
The response contract is per-endpoint, not repository-wide (D-120). The 129
delivered routes split **124 / 4 / 1**, and one endpoint's shape depends on a
query parameter:

| Shape | Live today | PHP terminates in | Java answers |
|---|---|---|---|
| **Envelope only** | **124** of the 129 | `ok()` / `fail()` (`apis/helpers/functions.php`) | D-074's JSON envelope — including `attendance/overall_report.php`, delivered by Wave 12.6.6c |
...
`theResponseShapePartitionMatchesTheCompletionPlan` pins the 124/4/1 arithmetic to
...
- **The three Item-12 endpoints** (C9, §6) — **delivered 2026-08-28.** Nothing
  ...
  `FINAL_COMPATIBLE` stands at 129 (§3.2), Item 13.0 having added `configs/get.php`.
