...
| `employees.update_bulk.chunk_replayed` at WARN | The line in the application log, with `tenant_id`, `first_row`, `rows` and the exception's class | A transaction writing part of an `update_bulk.php` sheet did not commit -- a row the database refused, or a re-read that no longer matched the validation -- and was halved and written again until the failing rows were alone (D-294). Expected, several lines per failing row, when a row fails at the database; the rows that failed are in the response's `failed` list. Frequent lines on sheets whose rows all succeed mean the batching regressed, and those sheets paid extra round trips. The line carries no row value |
| `employees.update_bulk.chunk_commit_failed` at WARN | The line in the application log, with `tenant_id`, `first_row`, `rows` and the exception's class; up to 500 rows of the response's `failed` list report `employee_update_failed` | The commit of a sheet's chunk failed, so whether its rows were written is unknown -- **they may have landed** although they are reported failed (D-294). They are not retried, which could write a shift assignment twice. Before resubmitting those rows, check the employees named in `first_row` onwards against the sheet; a repeat points at the database connection |
