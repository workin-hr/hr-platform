...
| Administrator deactivated but still active | Should be impossible for anything that reaches a controller: the session is revalidated on every such request. **Assets are the one exception (D-273)**: a request under `/admin/_assets/**` is not revalidated, so a deactivated administrator's browser may still be served a stylesheet or script. Those paths are `permitAll`, so serving one never depended on the caller being an administrator; the session itself is untouched and still present, only the repository check is skipped, and the session's absolute cap is still checked on those requests and still ends the session when it is past. Seeing one in the access log after a deactivation is expected, not a bug -- but it is also not evidence that the session ended, and an asset fetch still counts as session activity | `PlatformAdminSessionRevalidationFilter`; regression coverage in `PlatformAdminSessionRevalidationFilterTest` (`aStaticAssetIsServedWithoutRevalidating`, `anAssetRequestPastTheAbsoluteCapIsStillRefused`) and, for a page request, `PlatformAdminWebSessionTest.deactivatingAnAdministratorRefusesTheirNextPageRequest` |
...
| Every login attempt shares one `identifier_hash` | `platform_admin_login_attempts` shows the same hash for every caller | A proxy is in front and `server.forward-headers-strategy` is `none`, so every caller looks like the proxy and one guesser can lock out everyone. Behind `deploy/compose.tls.yaml` the strategy is already pinned to `native` and no variable changes it. Elsewhere, set it to `native` only with the port closed to everything but the proxy -- `running-the-backend.md` names the variable for each profile |
| `identifier_hash` values vary implausibly | Many distinct hashes, few real callers | The reverse: `native` while callers can reach the application without going through the proxy. A private or loopback source is believed, so each attempt can claim a fresh address and the miss budget is unspendable. Close that route, or set the strategy back to `none` until a proxy is the sole route (**R-049**) |
...
| App logins answering 429 for everyone | Users of the mobile and desktop apps told "Too many login attempts" though they have not failed eight times | D-289's address budget (60 misses per 15 minutes) is charged to `getRemoteAddr()`. With a proxy in front and `server.forward-headers-strategy` at `none`, every app user is the proxy's address, so sixty misses from anyone refuse everyone -- visible as `platform_admin_login_attempts` rows holding one hash for nearly every attempt, and 429s on `/apis/api/auth/login_*.php` in the access log. The fix is the forwarding strategy above, not the budget. A burst from one real address is a carrier NAT or a guesser; it clears when the window passes |
| One app user refused from every address | A user told "Too many login attempts" wherever they sign in from, their correct password included | D-289's phone-wide ceiling: forty misses on that phone within 15 minutes, from several addresses -- someone is guessing that account. The phone is keyed on its E.164 number (D-291), so misses in any spelling of it count, and a number typed nationally counts against every country it reads as (a Saudi `05...` also charges the Egyptian and Emirati readings). It clears when the window passes; eight misses from one address refuse only that address |
| A user who signs in by phone reports "user not found" for a number that is registered | The app login answering 401 for a known account | The stored row does not canonicalise under its own `country_code` (a number libphonenumber rejects, or a code naming no region), or it holds punctuation, which a lookup does not match (D-291, ADR-0020). Read the row and correct the stored phone to the number's national digits and dial code; the owner's 2026-09-26 profile found no such row. Java never stores a client's `country_code` as sent -- only a parsed dial code, NULL or the stored value -- and refuses a code-only write the stored phone does not hold under (`invalid_phone_number`), so a new such row comes from a PHP write (a padded, zero-prefixed or truncated code such as `+000000096` is the tell) |
| A user reports no WhatsApp code arrived, and the app said the phone was not found or not valid | `forgot_password` answering 404 `phone_not_found`, or `resend_otp` 400 `invalid_phone_number`, for a number that is registered | The number is in a country the product does not offer -- not an active `phone_countries` row and not +20/+966/+971 -- and OTPs are delivered only there (D-291). Activate the country in `phone_countries` if it should be offered |
| `Could not return an app login's throttle reservation` at WARN | The line in the application log, with a database exception | The database failed while `LegacyLoginThrottle` returned an attempt's rows after the login had run. The login was served as normal and the rows age out with the window; repeated lines point at the database, not the throttle. The line names neither the phone nor the address |
| `employees.update_bulk.chunk_replayed` at WARN | One line per chunk of an `update_bulk.php` sheet (up to 500 rows), with `tenant_id`, `first_row`, `rows`, `failed_transactions` and the exception classes | Some transaction writing that chunk did not commit -- a row the database refused, a re-read that no longer matched the validation, a deadlock or lock-wait timeout -- and its rows were halved, or below eight rows or on a lock written one by one, until the failing rows were alone (D-294). Expected when a row fails at the database; the rows that failed are in the response's `failed` list. Worst case, every row of a chunk failing: about 1.25 transactions per row, and `failed_transactions` near the row count. Lines on sheets whose rows all succeed mean the batching regressed; `ArrayIndexOutOfBoundsException` among the causes means a trigger now writes from `employee_shift_assignments` or `salary_contracts`, which Connector/J's bulk insert cannot handle -- set `useBulkStmtsForInserts=false` on the JDBC URL. The line carries no row value |
| `employees.update_bulk.chunk_commit_failed` at WARN | The line in the application log, with `tenant_id`, `first_row`, `rows` and the exception's class; up to 500 rows of the response's `failed` list (one, when a row written alone lost its commit) report `employee_update_failed` | The commit of a sheet's chunk, or of one row, failed, so whether its rows were written is unknown -- **they may have landed** although they are reported failed (D-294). They are not retried, which could write a shift assignment twice. Before resubmitting those rows, check the employees named in `first_row` onwards against the sheet; a repeat points at the database connection |
