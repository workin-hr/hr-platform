...
- `going-live-on-a-vps.md` — the cutover onto a server of your own: which stack to run,
  every value that changes and the file it lives in, the order, and how to tell it worked
- `provisioning-phase1-tables.md` — the one procedure for adding Phase 1 to a legacy database: what it adds and alters, the backup, the order, how to confirm each part, and the rollback
- `checking-against-the-live-database.md` — the backend in Docker against the MySQL PHP still serves: provisioning first, the three settings that differ against real data, and how the mobile, desktop and admin clients point at it
