...
### The surface runs on MySQL (D-162, and ADR-0017)

ADR-0015 does not say which database backs this surface, and the first
implementation assumed the PostgreSQL domain. D-162 made it run under both;
**ADR-0017 (2026-09-08) left one**, the legacy MariaDB. Legacy has a platform
admin web of its own, so a MySQL deployment without one would be a regression
against PHP rather than a deferral.
...
The company mapping was the one thing the two databases could not share --
PostgreSQL's `companies` has `name`, legacy's has `company_name` -- and
`PlatformAdminCompanyDirectory` exists for it. One implementation remains, the
legacy one; the interface stays because the company *actions* are written
against it.
